...
describe('RhemaConfigurationManager', () => {
  ...
  describe('updateConfiguration', () => {
    it('should update configuration with new values', () => {
      ...
      const config = configManager.getConfiguration();
      expect(config.formatting.indentSize).toBe(4); // Updated value
      expect(config.formatting.useSpaces).toBe(false); // Updated value
      expect(config.formatting.lineWidth).toBe(120); // Updated value
    });
    ...
    it('should merge configuration instead of replacing', () => {
      ...
      const config = configManager.getConfiguration();
      expect(config.formatting.indentSize).toBe(4); // Updated value
      expect(config.formatting.useSpaces).toBe(originalConfig.formatting.useSpaces); // Preserved
      expect(config.completion).toEqual(originalConfig.completion); // Preserved
    });
    ...
    it('should handle partial updates', () => {
      ...
      const config = configManager.getConfiguration();
      expect(config.performance.maxMemory).toBe(1000); // Updated value
      expect(config.performance.timeout).toBe(5000); // Default value preserved
    });
