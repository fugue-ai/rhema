...
pub async fn handle_integrations(cmd: IntegrationSubcommands) -> RhemaResult<()> {
    match cmd {
        ...
        IntegrationSubcommands::Test { name, all } => test_integrations(name, all).await,
        IntegrationSubcommands::Status { name, detailed } => {
            show_integration_status(name, detailed).await
        }
        IntegrationSubcommands::Remove { name, force } => remove_integration(name, force).await,
        IntegrationSubcommands::Execute {
            name,
            action,
            params,
...
async fn list_integrations(
    ...
    for integration in integrations {
        ...
        let status = integration
            .get_status()
            .await
            .unwrap_or_else(|_| IntegrationStatus {
                connected: false,
...
async fn test_integrations(name: Option<String>, all: bool) -> RhemaResult<()> {
    ...
    if all {
        ...
        return Err(RhemaError::ConfigError(
            "Please specify an integration name or use --all".to_string(),
        ));
    }
...
async fn remove_integration(name: String, force: bool) -> RhemaResult<()> {
    if !force {
        println!(
            "Are you sure you want to remove integration '{}'? (y/N)",
            name
        );
    ...
    if config_path.exists() {
        ...
        println!(
            "{}",
            format!("Integration '{}' removed successfully.", name)
                .green()
                .bold()
        );
    } else {
...
async fn execute_integration_action(
    ...
    if !metadata.capabilities.contains(&action) {
        return Err(RhemaError::ConfigError(format!(
            "Action '{}' not supported by integration '{}'. Available actions: {}",
            action,
            name,
    ...
    println!(
        "{}",
        format!("Executing '{}' on integration '{}'", action, name)
            .blue()
            .bold()
    );
    ...
    match metadata.integration_type {
            ...
            if let Some(jira_integration) = integration.as_ref().downcast_ref::<JiraIntegration>() {
                match action.as_str() {
                    "create_issue" => {
                        ...
                        let issue_type = parameters
                            .get("issue_type")
                            .and_then(|v| v.as_str())
                            .unwrap_or("Task");

                        let result = jira_integration
        ...
        _ => {
            return Err(RhemaError::ConfigError(format!(
                "Integration type '{}' not yet supported for execution",
                metadata.integration_type.to_string()
            )))
        }
    }
