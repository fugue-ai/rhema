...
use rhema_git::{git::security::Operation, git_hooks::HookType, utils::AdvancedGitIntegration};
use std::fs;
...
"#,
    ...
    println!(
        "Evolution tracking returned {} entries",
        evolution.entries.len()
    );
    ...
    println!(
        "Evolution report: total_commits={}, changes_by_type={:?}",
