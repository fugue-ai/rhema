...
fn benchmark_agent_registration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let stats = run_benchmark_iterations(10, || {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("bench-agent-{}", rand::random::<u32>()),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])
    })?;
...
fn benchmark_agent_listing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..50 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("list-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])?;
    }
    ...
    let stats = run_benchmark_iterations(20, || {
        benchmark_coordination_command(&["agent", "list"])
    })?;
...
fn benchmark_agent_listing_with_filters() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..100 {
        let agent_type = if i % 3 == 0 { "CodeReviewAgent" } else { "TestAgent" };
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("filter-bench-agent-{}", i),
            "--type", agent_type,
            "--scope", "benchmarking"
        ])?;
    }
...
fn benchmark_agent_broadcast() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..20 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("broadcast-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])?;
    }
...
fn benchmark_session_creation() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..10 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("session-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])?;
    }
...
fn benchmark_session_listing() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let stats = run_benchmark_iterations(20, || {
        benchmark_coordination_command(&["session", "list"])
    })?;
...
fn benchmark_system_stats() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..25 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("stats-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])?;
    }
    ...
    let stats = run_benchmark_iterations(25, || {
        benchmark_coordination_command(&["system", "stats"])
    })?;
...
fn benchmark_system_stats_detailed() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..50 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("detailed-stats-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "benchmarking"
        ])?;
    }
...
fn benchmark_health_check() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let stats = run_benchmark_iterations(30, || {
        benchmark_coordination_command(&["system", "health"])
    })?;
...
fn benchmark_high_load_agent_registration() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..100 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("load-bench-agent-{}", i),
            "--type", "TestAgent",
            "--scope", "load-testing"
        ])?;
    }
...
fn benchmark_memory_usage_under_load() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    for i in 0..50 {
        benchmark_coordination_command(&[
            "agent", "register",
            "--name", &format!("memory-bench-agent-{}", i),
            "--type", "TestAgent",
        ...
        for j in 0..10 {
            benchmark_coordination_command(&[
                "agent", "send-message",
                "--to", &format!("agent-{:03}", i + 1),
                &format!("Memory test message {} from agent {}", j, i),
                "--message-type", "Test",
                "--priority", "Normal",
                "--payload", &format!("{{\"test\": \"memory\", \"agent\": {}, \"message\": {}}}", i, j)
            ])?;
        }
    }
...
fn benchmark_stress_test_rapid_operations() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("Success rate: {:.2}%", (success_count as f64 / 500.0) * 100.0);
    println!("Total time: {:?}", total_duration);
    println!("Operations per second: {:.2}", 500.0 / total_duration.as_secs_f64());
    println!("===========================");
