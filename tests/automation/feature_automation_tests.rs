...
fn test_validate_feature_branch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let context = _fixture
        .automation_manager
        .setup_feature_context("test-feature", "develop")?;

    // Test validation
    let validation_result = _fixture
        .automation_manager
    ...
    assert_eq!(
        ...
        rhema_git::git::feature_automation::ValidationStatus::Passed
    );
...
fn test_validate_feature_branch_nonexistent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let validation_result = _fixture
        .automation_manager
        .validate_feature_branch("nonexistent-branch")?;

    // Should fail validation
    assert!(!validation_result.success);
    assert!(matches!(
        validation_result.status,
        rhema_git::git::feature_automation::ValidationStatus::Failed(_)
    ));
...
fn test_merge_feature_branch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test merging
    let merge_result = _fixture
        .automation_manager
        .merge_feature_branch(&context.branch_name, "develop")?;
...
fn test_cleanup_feature_branch() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test cleanup
    let cleanup_result = _fixture
        .automation_manager
        .cleanup_feature_branch(&context.branch_name)?;
...
fn test_feature_context_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deserialized_context: rhema_git::git::feature_automation::FeatureContext =
    ...
    assert_eq!(
        context.config.context_type,
        deserialized_context.config.context_type
    );
...
fn test_validation_result_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deserialized_result: rhema_git::git::feature_automation::ValidationResult =
        serde_json::from_str(&json)?;
...
fn test_merge_result_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deserialized_result: rhema_git::git::feature_automation::MergeResult =
        serde_json::from_str(&json)?;

    assert_eq!(merge_result.success, deserialized_result.success);
    assert_eq!(
        merge_result.target_branch,
        deserialized_result.target_branch
    );
    assert_eq!(
        merge_result.source_branch,
        deserialized_result.source_branch
    );
    assert_eq!(
        merge_result.conflicts.len(),
        deserialized_result.conflicts.len()
    );
    assert_eq!(
        merge_result.messages.len(),
        deserialized_result.messages.len()
    );
...
fn test_cleanup_result_serialization() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let deserialized_result: rhema_git::git::feature_automation::CleanupResult =
        serde_json::from_str(&json)?;
...
fn test_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = _fixture
        .automation_manager
        .setup_feature_context("test", "invalid-base");
    assert!(result.is_err());
