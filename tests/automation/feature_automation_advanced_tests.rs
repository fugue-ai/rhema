...
fn test_health_checks_branch_health() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let validation_result = fixture
        .automation_manager
        .validate_feature_branch("nonexistent");
...
"#,
    ...
    assert!(validation_result
        .errors
        .iter()
        .any(|e| e.contains("placeholder version")));
...
"#,
    ...
    assert!(validation_result
        .errors
        .iter()
        .any(|e| e.contains("Security issue found")));
...
"#,
    ...
    let error_messages: Vec<&str> = validation_result
        .errors
        .iter()
        .map(|s| s.as_str())
        .collect();
