...
"#;
    ...
    assert!(scope.is_ok(), "Should be able to load scope from .rhema/rhema.yaml");
    ...
    let scope = Scope::new(rhema_dir.clone());
    assert!(scope.is_ok(), "Should be able to load scope from .rhema/scope.yaml");
    ...
    let scope = Scope::new(rhema_dir.clone());
    assert!(scope.is_ok(), "Should be able to load scope from parent/rhema.yaml");
    ...
    let scope = Scope::new(rhema_dir.clone());
    assert!(scope.is_ok(), "Should be able to load scope from parent/scope.yaml");
    ...
    assert_eq!(rhema_file.file_name().unwrap(), "rhema.yaml", "Should prefer rhema.yaml over scope.yaml in same directory");
    ...
    assert_eq!(rhema_file.file_name().unwrap(), "rhema.yaml", "Should prefer rhema.yaml in .rhema over scope.yaml in parent");
} 