...
async fn test_semantic_search_with_cache(engine: &UnifiedKnowledgeEngine) {
    ...
    let search_results = engine
        .search_semantic("machine learning algorithms", 3)
        .await
    ...
    for result in &search_results {
        ...
        assert!(
            !result.cache_key.is_empty(),
            "Results should have cache keys"
        );
    }
...
async fn test_file_watching_proactive_indexing(engine: &UnifiedKnowledgeEngine) {
    ...
    file_watcher
        .watch_file(test_file_path.clone())
        .await
    ...
    tokio::fs::write(&test_file_path, updated_content)
        .await
...
async fn test_usage_analysis_intelligent_warming(_engine: &UnifiedKnowledgeEngine) {
    ...
    usage_analyzer
        .record_access("test:pattern:key1", Some("agent1"), Some("workflow1"))
        .await
    ...
    usage_analyzer
        .record_access("test:pattern:key2", Some("agent1"), Some("workflow1"))
        .await
        .expect("Failed to record access pattern");
    ...
    let session_context = AgentSessionContext {
        ...
        preferences: Default::default(),
        cache_keys: vec![
            "test:pattern:key1".to_string(),
            "test:pattern:key2".to_string(),
        ],
    };

    // Analyze agent session
    let session_analysis = usage_analyzer
        .analyze_agent_session("agent1", &session_context)
        .await
...
async fn test_suggestion_engine_context_recommendations(_engine: &UnifiedKnowledgeEngine) {
    ...
    assert!(
        ...
        "Should generate suggestions for agent"
    );
    ...
    for suggestion in &suggestions {
        ...
        assert!(
            ...
            "Suggestions should have titles"
        );
        assert!(
            !suggestion.description.is_empty(),
            "Suggestions should have descriptions"
        );
    }
    ...
    if let Some(suggestion) = suggestions.first() {
        suggestion_engine
            ...
            .await
            .expect("Failed to record suggestion event");
    }

    // Get suggestion statistics
    let stats = suggestion_engine
        .get_suggestion_stats("developer_agent")
        .await;
    assert!(
...
async fn test_performance_monitoring_optimization(engine: &UnifiedKnowledgeEngine) {
    ...
    assert!(
        metrics.cache_metrics.hit_count >= 0,
        "Cache metrics should be valid"
    );
    assert!(
        metrics.search_metrics.total_searches >= 0,
        "Search metrics should be valid"
    );
    assert!(
        metrics.synthesis_metrics.total_syntheses >= 0,
        "Synthesis metrics should be valid"
    );
