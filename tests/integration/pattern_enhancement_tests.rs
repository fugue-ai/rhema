        ...
        for (step_index, step) in self.execution_steps.iter().enumerate() {
            ...
            if self.should_fail && step == "execute" {
                return Err(PatternError::ExecutionError(format!(
                    "Enhanced pattern {} failed at step: {}", self.id, step
                )));
            }
            ...
            if self.should_recover && step == "execute" && self.recovery_attempts < 2 {
                ...
                return Err(PatternError::ExecutionError(format!(
                    "Enhanced pattern {} failed but can recover at step: {}", self.id, step
                )));
            }
        }
        ...
        Ok(PatternResult {
            ...
            data: HashMap::from([
                ("execution_steps".to_string(), serde_json::Value::Array(
                    self.execution_steps.iter().map(|s| serde_json::Value::String(s.clone())).collect()
                )),
                ("current_step".to_string(), serde_json::Value::Number(current_step.into())),
...
async fn test_enhanced_pattern_monitoring_events() {
    ...
    let has_started = events.iter().any(|event| {
        matches!(event, rhema_ai::agent::patterns::MonitoringEvent::PatternStarted { .. })
    });
    assert!(has_started);
    
    let has_completed = events.iter().any(|event| {
        matches!(event, rhema_ai::agent::patterns::MonitoringEvent::PatternCompleted { .. })
    });
...
async fn test_enhanced_pattern_performance_profiling() {
    ...
    assert!(pattern_result.performance_metrics.total_execution_time_seconds > 0.0);
