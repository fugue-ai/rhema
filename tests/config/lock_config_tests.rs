...
fn test_lock_config_creation_and_defaults() {
    ...
    assert_eq!(config.resolution.default_strategy, ResolutionStrategy::Latest);
    ...
    assert_eq!(config.conflict_resolution.primary_strategy, ConflictResolutionStrategy::LatestCompatible);
    ...
    assert_eq!(config.update_policies.update_frequency, UpdateFrequency::OnDemand);
...
fn test_resolution_config() {
    ...
    config.strategy_overrides.insert("test-dep".to_string(), ResolutionStrategy::Conservative);
    assert_eq!(config.strategy_overrides.get("test-dep"), Some(&ResolutionStrategy::Conservative));
    
    // Test version constraint config
    assert_eq!(config.version_constraints.default_type, ConstraintType::Caret);
...
fn test_conflict_resolution_config() {
    ...
    assert!(config.fallback_strategies.contains(&ConflictResolutionStrategy::SmartSelection));
    assert!(config.fallback_strategies.contains(&ConflictResolutionStrategy::Conservative));
    
    // Test dependency type preferences
    config.dependency_type_preferences.insert("production".to_string(), ConflictResolutionStrategy::Conservative);
    assert_eq!(
...
fn test_update_policies_config() {
    ...
    assert!(matches!(config.notifications.channels[0], NotificationChannel::Console));
...
fn test_performance_config() {
    ...
    assert_eq!(config.optimization.optimization_level, OptimizationLevel::Standard);
    ...
    assert_eq!(config.monitoring.alert_thresholds.memory_usage_threshold_percent, 80);
    assert_eq!(config.monitoring.alert_thresholds.cpu_usage_threshold_percent, 80);
    assert_eq!(config.monitoring.alert_thresholds.response_time_threshold_ms, 5000);
    assert_eq!(config.monitoring.alert_thresholds.error_rate_threshold_percent, 5);
    assert_eq!(config.monitoring.alert_thresholds.cache_hit_rate_threshold_percent, 80);
}
...
fn test_config_serialization() {
    ...
    assert_eq!(config.resolution.default_strategy, deserialized.resolution.default_strategy);
    assert_eq!(config.conflict_resolution.primary_strategy, deserialized.conflict_resolution.primary_strategy);
...
fn test_environment_config() {
    ...
    assert_eq!(retrieved_config.description, Some("Production environment".to_string()));
    assert_eq!(retrieved_config.resolution_strategy, Some(ResolutionStrategy::Conservative));
    assert_eq!(retrieved_config.conflict_resolution_strategy, Some(ConflictResolutionStrategy::Conservative));
...
fn test_config_value_setting() {
    ...
    config.set_value("resolution.prefer_stable", serde_json::json!(false)).unwrap();
    assert!(!config.resolution.prefer_stable);
    ...
    config.set_value("resolution.max_depth", serde_json::json!(15)).unwrap();
    assert_eq!(config.resolution.max_depth, 15);
    
    // Test setting enum values
    config.set_value("resolution.default_strategy", serde_json::json!("Conservative")).unwrap();
    assert_eq!(config.resolution.default_strategy, ResolutionStrategy::Conservative);
    
    config.set_value("conflict_resolution.primary_strategy", serde_json::json!("SmartSelection")).unwrap();
    assert_eq!(config.conflict_resolution.primary_strategy, ConflictResolutionStrategy::SmartSelection);
    
    // Test setting update policy values
    config.set_value("update_policies.auto_update_enabled", serde_json::json!(true)).unwrap();
    assert!(config.update_policies.auto_update_enabled);
    
    config.set_value("update_policies.update_frequency", serde_json::json!("Daily")).unwrap();
    assert_eq!(config.update_policies.update_frequency, UpdateFrequency::Daily);
    
    // Test setting validation values
    config.set_value("validation.validation_enabled", serde_json::json!(false)).unwrap();
    assert!(!config.validation.validation_enabled);
    ...
    config.set_value("performance.cache.size_limit_mb", serde_json::json!(500)).unwrap();
...
fn test_config_export_import() {
    ...
    assert_eq!(config.resolution.default_strategy, new_config.resolution.default_strategy);
    ...
    assert_eq!(config.conflict_resolution.primary_strategy, new_config2.conflict_resolution.primary_strategy);
...
fn test_config_file_operations() {
    ...
    assert_eq!(loaded_config.resolution.default_strategy, ResolutionStrategy::Conservative);
    assert_eq!(loaded_config.conflict_resolution.compatibility_threshold, 0.9);
...
fn test_custom_validation_rules() {
    ...
    config.validation.custom_rules.insert("test_rule".to_string(), custom_rule);
...
fn test_notification_channels() {
    ...
    assert!(matches!(config.channels[3], NotificationChannel::Webhook(_)));
...
fn test_validation_severities() {
    ...
    for severity in severities {
        ...
        let deserialized: rhema_config::lock::CustomValidationRule = serde_json::from_str(&json).unwrap();
