...
use rhema::mcp::{
    McpConfig, RhemaMcpServer, ContextProviderExt,
    SdkResource, SdkTool, SdkPrompt, SdkToolResult,
    ContextProvider, CacheManager, FileWatcher, AuthManager,
};
...
async fn test_mcp_server_creation() {
    ...
    let cache_manager = Arc::new(CacheManager::new(&McpConfig::default().cache).await.unwrap());
    let file_watcher = Arc::new(FileWatcher::new(&McpConfig::default().watcher, repo_root).await.unwrap());
    let auth_manager = Arc::new(AuthManager::new(&McpConfig::default().auth).unwrap());
    
    // Create MCP server
    let server = RhemaMcpServer::new(
        context_provider,
        cache_manager,
        file_watcher,
        auth_manager,
...
async fn test_mcp_tool_execution() {
    ...
    // Test unknown tool
    let result = server.execute_tool("unknown_tool", serde_json::json!({})).await;
