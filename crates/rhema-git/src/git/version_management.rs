...
impl VersionManager {
    ...
    pub fn get_current_version(&self) -> RhemaResult<String> {
        for version_file in &self.config.version_files {
            if let Ok(version) =
                self.read_version_from_file(&version_file.path, &version_file.file_type)
            {
                return Ok(version);
            }
        }
        ...
        Err(RhemaError::ConfigError(
            "No version found in any version file".to_string(),
        ))
    }
    ...
    pub async fn bump_version(
        ...
        // Generate changelog
        if self.config.changelog.file_path.exists()
            || self
                .config
                .changelog
                .file_path
    ...
    pub async fn generate_release_notes(&self, version: &str) -> RhemaResult<()> {
        ...
        // Write release notes
        let release_notes_path = self
            .config
            .release_notes
            .directory
            .join(format!("release-{}.md", version));
            ...
            if versions.len() > 1 {
                ...
                for (i, ver) in versions.iter().enumerate().skip(1) {
                    if ver != first_version {
                        errors.push(format!(
                            "Version inconsistency detected: {} vs {}",
                            first_version, ver
                        ));
                    }
                }
    ...
    fn parse_cargo_toml_version(&self, content: &str) -> RhemaResult<String> {
        let version_regex = Regex::new(r#"version\s*=\s*"([^"]+)""#)
            .map_err(|e| RhemaError::ConfigError(format!("Invalid regex: {}", e)))?;
        if let Some(captures) = version_regex.captures(content) {
            Ok(captures[1].to_string())
        } else {
            Err(RhemaError::ConfigError(
                "Version not found in Cargo.toml".to_string(),
            ))
        }
    }
    ...
    fn parse_package_json_version(&self, content: &str) -> RhemaResult<String> {
        ...
        if let Some(captures) = version_regex.captures(content) {
            ...
            Err(RhemaError::ConfigError(
                "Version not found in package.json".to_string(),
            ))
        }
    }
    ...
    fn update_version_in_file(
        ...
        let updated_content = match file_type {
            VersionFileType::CargoToml => {
                self.update_cargo_toml_version(&content, old_version, new_version)?
            }
            VersionFileType::PackageJson => {
                self.update_package_json_version(&content, old_version, new_version)?
            }
            VersionFileType::VersionFile => {
                self.update_version_file_content(&content, old_version, new_version)?
            }
            VersionFileType::Custom(_) => {
                self.update_custom_version_file(&content, file_type, old_version, new_version)?
            }
        };
    ...
    fn update_version_file_content(
        &self,
        content: &str,
        old_version: &str,
        new_version: &str,
        ...
        if regex.is_match(content) {
            ...
            Err(RhemaError::ConfigError(
                "Version not found in version file".to_string(),
            ))
        }
    }

    /// Update version in custom file type
    fn update_custom_version_file(
        &self,
        content: &str,
        file_type: &VersionFileType,
        old_version: &str,
    ...
    fn bump_major_version(&self, version: &str) -> RhemaResult<String> {
        let mut ver = Version::parse(version)
            .map_err(|e| RhemaError::ConfigError(format!("Invalid version format: {}", e)))?;
        ver.major += 1;
        ver.minor = 0;
        ver.patch = 0;
    ...
    fn parse_commit_info(&self, commit: &Commit) -> RhemaResult<CommitInfo> {
        ...
        let (commit_type, scope, description, breaking_change) =
    ...
    fn parse_commit_message(&self, message: &str) -> (CommitType, Option<String>, String, bool) {
        ...
        if let Some(captures) = conventional_regex.captures(message) {
            ...
            let breaking_change =
                message_lower.contains("breaking") || message_lower.contains("!breaking");
    ...
    async fn format_markdown_changelog(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
    ...
    async fn format_json_changelog(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
    ...
    async fn format_custom_changelog(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
    ...
    async fn format_release_notes(
        ...
        match self.config.release_notes.format {
            ...
            ReleaseNotesFormat::Custom(_) => {
                self.format_custom_release_notes(version, commits).await
            }
        }
    }

    /// Format markdown release notes
    async fn format_markdown_release_notes(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
        ...
        if self.config.release_notes.include_breaking_changes {
            let breaking_changes: Vec<&CommitInfo> =
        ...
        // Features section
        let features: Vec<&CommitInfo> = commits
            .iter()
            .filter(|c| matches!(c.commit_type, CommitType::Feature))
            .collect();
        ...
        // Bug fixes section
        let fixes: Vec<&CommitInfo> = commits
            .iter()
            .filter(|c| matches!(c.commit_type, CommitType::Fix))
            .collect();
        if !fixes.is_empty() {
        ...
        let other_changes: Vec<&CommitInfo> = commits
            .iter()
            .filter(|c| {
                !matches!(c.commit_type, CommitType::Feature | CommitType::Fix)
                    && !c.breaking_change
            })
            .collect();
        ...
        if self.config.release_notes.include_security_notes {
            let security_commits: Vec<&CommitInfo> = commits
                .iter()
                .filter(|c| {
                    c.description.to_lowercase().contains("security")
    ...
    async fn format_html_release_notes(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
    ...
    async fn format_custom_release_notes(
        &self,
        version: &str,
        commits: &[CommitInfo],
    ) -> RhemaResult<String> {
...
pub fn default_version_management_config() -> VersionManagementConfig {
    VersionManagementConfig {
        ...
        commit_patterns: CommitPatterns {
            ...
            breaking_change: vec![r"!breaking".to_string(), r"BREAKING CHANGE".to_string()],
            ignore: vec![
                r"docs".to_string(),
                r"style".to_string(),
                r"chore".to_string(),
