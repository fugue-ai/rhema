...
pub use cache::{
    UnifiedCacheManager, CacheMonitor, CacheOptimizer, CacheValidator, 
    AdaptiveEvictionPolicy, CacheMetrics, CachePerformanceReport,
    UnifiedCacheStats
};

// Engine module exports
pub use engine::{
    UnifiedKnowledgeEngine, DistributedRAGCache, ConnectionPool,
    ProactiveContextManager, FileWatcher, UsageAnalyzer, SuggestionEngine,
    FileWatchInfo, SuggestionEngineConfig
};

// Types module exports
pub use types::{
    KnowledgeError, CacheTier, SemanticInfo, SearchResultMetadata,
    CacheInfo, AgentSessionContext, WorkflowContext, WorkflowType,
    ...
    RAGConfig, VectorStoreConfig, VectorStoreType, DistanceMetric,
    SemanticSearchConfig, CacheConfig, MemoryConfig,
    DiskConfig, NetworkConfig, LifecycleConfig, PerformanceConfig,
    ProactiveConfig, MonitoringConfig, CacheMetrics as TypesCacheMetrics,
    SearchMetrics, SynthesisMetrics, ProactiveMetrics, PerformanceMetrics,
    UnifiedCacheResult, CacheEntryMetadata, AccessPatterns, TemporalPattern,
    CompressionAlgorithm, ContentType, EvictionPolicy, KnowledgeResult,
...
pub use proactive::{
    ProactiveContextManager as ProactiveManager, FileWatcher as ProactiveFileWatcher,
    UsageAnalyzer as ProactiveUsageAnalyzer, SuggestionEngine as ProactiveSuggestionEngine,
    FileWatchInfo as ProactiveFileWatchInfo, SuggestionEngineConfig as ProactiveSuggestionConfig,
    ProactiveConfig as ProactiveEngineConfig
};
...
mod tests {
    use super::*;
    use crate::types::*;
    use crate::embedding::{EmbeddingManager, EmbeddingCompressionType};
    use crate::indexing::{SemanticIndexer, IndexingConfig, IndexingMetadata, IndexCleanupConfig};
    use crate::engine::{UnifiedKnowledgeEngine, ProactiveContextManager};
    ...
    async fn test_search_enhancements() {
        ...
        // Test reranking
        let results = search_engine.search_with_reranking("test query", 5).await.unwrap();
        ...
        // Test hybrid search
        let hybrid_results = search_engine.search_hybrid("test query", 5, 0.7).await.unwrap();
    ...
    async fn test_embedding_enhancements() {
        ...
        embedding_manager.cache_embedding("test_key", &embedding, None).await.unwrap();
        
        // Test cached embedding retrieval
        let cached = embedding_manager.get_cached_embedding("test_key", None).await.unwrap();
        assert!(cached.is_some());
        
        // Test embedding validation
        let validation = embedding_manager.validate_embedding(&embedding, None).await.unwrap();
        assert!(validation.is_valid);
        
        // Test embedding compression
        let compressed = embedding_manager.compress_embedding(&embedding, EmbeddingCompressionType::Quantization).await.unwrap();
        assert_eq!(compressed.original_dimension, embedding.len());
        
        // Test embedding decompression
        let decompressed = embedding_manager.decompress_embedding(&compressed).await.unwrap();
        assert_eq!(decompressed.len(), embedding.len());
        
        // Test embedding versioning
        let versioned = embedding_manager.version_embedding(&embedding, "1.0.0").await.unwrap();
    ...
    async fn test_indexing_enhancements() {
        ...
        let result = indexer.incremental_index(content, metadata, "old_hash").await.unwrap();
        assert!(!result.is_empty());
        
        // Test index validation
        let validation = indexer.validate_index(&PathBuf::from("test.txt")).await.unwrap();
        assert!(validation.is_valid);
        ...
        // Test index recovery
        let recovery = indexer.recover_from_failure(&PathBuf::from("test.txt")).await.unwrap();
    ...
    async fn test_error_handling() {
        ...
        let result = embedding_manager.validate_embedding(&invalid_embedding, None).await;
        assert!(result.is_ok());
