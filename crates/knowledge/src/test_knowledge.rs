...
mod tests {
    ...
    use crate::vector::{VectorStoreWrapper, MockVectorStore as VectorMockVectorStore, VectorStore};
    use crate::storage::{StorageManager, StorageConfig, StorageMetadata};
    use crate::cache::{UnifiedCacheManager, UnifiedCacheConfig};
    ...
    async fn test_storage_functionality() {
        ...
        let store_result = storage_manager.store("test_key", &test_data, metadata).await;
        assert!(store_result.is_ok());
    ...
    async fn test_knowledge_engine_integration() {
        ...
        let engine = UnifiedKnowledgeEngine::new(UnifiedEngineConfig::default()).await.unwrap();
