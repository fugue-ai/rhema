...
async fn await_wizard_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    wizard.answer_question(
        "repo_url",
        json!("https://github.com/example/rhema-project"),
    )?;
    ...
    println!(
        "   - Steps completed: {}",
        result.statistics.steps_completed
    );
    println!(
        "   - Questions answered: {}",
        result.statistics.questions_answered
    );
    println!(
        "   - Time taken: {} seconds",
        result.statistics.time_taken_seconds
    );
...
async fn await_feedback_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        feedback.summary.high_priority_issues
    );
    println!(
        "   - Medium priority issues: {}",
        feedback.summary.medium_priority_issues
    );
    println!("   - Suggestions: {}", feedback.summary.total_suggestions);
...
async fn await_integration_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    wizard.answer_question(
        "repo_url",
        json!("https://github.com/example/integration-test"),
    )?;
    wizard.answer_question("enable_encryption", json!(true))?;
    wizard.answer_question("enable_access_control", json!(true))?;
    wizard.answer_question("enable_audit_logging", json!(true))?;
    ...
    println!(
        "   ✅ Configuration created with {} questions answered",
        wizard_result.statistics.questions_answered
    );
    ...
    println!(
        "      - Sections: {}",
        doc_result.documentation.sections.len()
    );
    ...
    println!(
        "   ✅ Wizard: {} steps completed in {} seconds",
        wizard_result.statistics.steps_completed, wizard_result.statistics.time_taken_seconds
    );
    println!(
        "   ✅ Feedback: {} suggestions with {}% health score",
        feedback.summary.total_suggestions, feedback.summary.health_score
    );
    println!(
        "   ✅ Documentation: {} sections generated in {}ms",
        doc_result.statistics.sections_generated, doc_result.statistics.generation_time_ms
    );
