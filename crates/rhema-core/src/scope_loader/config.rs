...
impl GlobalScopeLoaderConfig {
    ...
    pub fn save_to_file(&self, path: &Path) -> RhemaResult<()> {
        ...
        let content = serde_yaml::to_string(self).map_err(|e| crate::RhemaError::InvalidYaml {
            file: path.display().to_string(),
            message: e.to_string(),
        })?;
    ...
    pub fn load_global() -> RhemaResult<Self> {
        let config_dir = dirs::config_dir()
            .ok_or_else(|| {
                crate::RhemaError::ConfigError("Could not determine config directory".to_string())
            })?
            .join("rhema");
            ...
            if let Some(threshold) = config.confidence_threshold {
                if threshold < 0.0 || threshold > 1.0 {
                    return Err(crate::RhemaError::ConfigError(format!(
                        "Plugin {} confidence threshold must be between 0.0 and 1.0",
                        name
                    )));
                }
            }
...
impl ScopeLoaderConfigManager {
    ...
    pub fn update_project_config(
        &mut self,
        project_root: &Path,
        config: GlobalScopeLoaderConfig,
    ) -> RhemaResult<()> {
        config.validate()?;
