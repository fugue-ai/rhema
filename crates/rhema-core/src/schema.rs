            ...
            for block in blocks {
                match block.block_type {
                    ...
                    CompositionBlockType::Switch => {
                        ...
                        if let Some(_condition) = &block.condition {
                            let block_content =
                                self.substitute_variables_with_map(&block.content, variables);
                            result.push_str(&block_content);
                        }
...
impl LockedDependency {
    ...
    pub fn calculate_checksum(&self) -> String {
        ...
        // Hash custom metadata if present
        if !self.custom.is_empty() {
            // Sort custom fields for deterministic hashing
            let mut custom_keys: Vec<_> = self.custom.keys().collect();
            custom_keys.sort();
            for key in custom_keys {
                hasher.update(key.as_bytes());
                if let Some(value) = self.custom.get(key) {
                    // Convert serde_yaml::Value to string for hashing
...
impl Validatable for ScopeDependency {
    fn validate(&self) -> crate::RhemaResult<()> {
        ...
        if let Some(version) = &self.version {
            if !version.trim().is_empty() {
                // Basic version constraint validation - check for common patterns
                let version = version.trim();
                if !version.chars().all(|c| c.is_alphanumeric() || matches!(c, '.' | ',' | '<' | '>' | '=' | '~' | '^' | '!' | '-' | '+')) {
                    return Err(crate::RhemaError::ValidationError(format!(
    ...
    fn validate_schema_version(&self) -> crate::RhemaResult<()> {
        // ScopeDependency doesn't have its own schema version
        Ok(())
    }

    fn validate_cross_fields(&self) -> crate::RhemaResult<()> {
        // Validate that path is a valid relative path
        if self.path.contains("..") {
            return Err(crate::RhemaError::ValidationError(
                "Dependency path cannot contain '..' for security reasons".to_string(),
            ));
        }

        // Validate that path doesn't start with absolute path indicators
        if self.path.starts_with('/') || self.path.starts_with('\\') {
            return Err(crate::RhemaError::ValidationError(
                "Dependency path must be relative, not absolute".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_scope_dependency_validation() {
        // Test valid dependency
        let valid_dep = ScopeDependency {
        ...
        // Test invalid path
        let invalid_path_dep = ScopeDependency {
            path: "".to_string(),
            dependency_type: "required".to_string(),
            version: None,
        };
        assert!(invalid_path_dep.validate().is_err());

        // Test invalid dependency type
        let invalid_type_dep = ScopeDependency {
            path: "crates/rhema-core".to_string(),
            dependency_type: "invalid".to_string(),
            version: None,
        };
        assert!(invalid_type_dep.validate().is_err());

        // Test invalid version constraint
        let invalid_version_dep = ScopeDependency {
            path: "crates/rhema-core".to_string(),
            dependency_type: "required".to_string(),
            version: Some("1.0.0@#$%".to_string()),
        };
        assert!(invalid_version_dep.validate().is_err());

        // Test path with security issue
        let security_issue_dep = ScopeDependency {
            path: "../external".to_string(),
            dependency_type: "required".to_string(),
            version: None,
        };
        // Test cross-field validation specifically
        assert!(security_issue_dep.validate_cross_fields().is_err());
    }

    #[test]
    fn test_checksum_calculation() {
        // Test RhemaLock checksum
        let lock = RhemaLock::new("test");
        let checksum1 = lock.calculate_checksum();
        assert!(!checksum1.is_empty());
        assert!(checksum1.len() == 64); // SHA256 hex string length

        // Test that checksum is deterministic
        let checksum2 = lock.calculate_checksum();
        assert_eq!(checksum1, checksum2);

        // Test LockedDependency checksum
        let mut dep = LockedDependency::new("1.0.0", "crates/rhema-core", DependencyType::Required);
        let dep_checksum1 = dep.calculate_checksum();
        assert!(!dep_checksum1.is_empty());
        assert!(dep_checksum1.len() == 64);

        // Test that dependency checksum is deterministic
        let dep_checksum2 = dep.calculate_checksum();
        assert_eq!(dep_checksum1, dep_checksum2);

        // Test that checksum changes when dependency changes
        dep.version = "2.0.0".to_string();
        let dep_checksum3 = dep.calculate_checksum();
        assert_ne!(dep_checksum1, dep_checksum3);
    }
}
