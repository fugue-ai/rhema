...
use rhema_query::query::{
    Condition, ConditionValue, CqlQuery, LogicalOperator, Operator, OrderBy, OrderDirection,
};
...
fn main() -> RhemaResult<()> {
    ...
    let complex_query = CqlQuery {
        query: "SELECT todos WHERE status='pending' AND priority='high' ORDER BY created_at DESC"
            .to_string(),
        target: "todos".to_string(),
        yaml_path: None,
        conditions: vec![
    ...
    println!(
        "  Order by: {:?}",
        complex_query.order_by.as_ref().map(|o| o.len())
    );
    ...
    let operators_query = CqlQuery {
        query: "SELECT todos WHERE priority > 5 AND status IN ('pending', 'in_progress')"
            .to_string(),
        target: "todos".to_string(),
        yaml_path: None,
        conditions: vec![
    ...
    println!(
        ...
        operators_query.conditions[0].operator
    );
    ...
    let logical_query = CqlQuery {
        query: "SELECT todos WHERE (status='pending' OR status='in_progress') AND priority='high'"
            .to_string(),
        target: "todos".to_string(),
        yaml_path: None,
        conditions: vec![
