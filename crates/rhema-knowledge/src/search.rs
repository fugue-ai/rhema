...
impl SemanticSearchEngine {
    ...
    async fn rerank_results(
        ...
        for result in &mut reranked_results {
            ...
            new_score *= self
                .calculate_semantic_tag_boost(&result.semantic_tags, query)
                .await?;
    ...
    async fn search_exact_keyword(
        ...
        self.convert_to_semantic_results(vector_results, keyword)
            .await
    }
    ...
    async fn calculate_recency_boost(
        &self,
    ...
    async fn calculate_content_type_boost(
        &self,
        ...
        match result.metadata.source_type {
            ...
            ContentType::Decision => {
                if query_lower.contains("decision")
                    || query_lower.contains("why")
                    || query_lower.contains("rationale")
                {
                    Ok(1.4)
                } else {
                    Ok(1.0)
    ...
    async fn calculate_semantic_tag_boost(
        &self,
        tags: &[String],
        query: &str,
    ) -> KnowledgeResult<f32> {
