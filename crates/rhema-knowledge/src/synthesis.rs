...
use crate::types::{
    ContentType, KnowledgeResult, KnowledgeSynthesis, SemanticResult,
    SynthesisMethod, SynthesisMetadata,
};
use chrono::Datelike;
...
impl KnowledgeSynthesizer {
    ...
    pub async fn synthesize(&self, topic: &str, scope_path: Option<&str>) -> KnowledgeResult<KnowledgeSynthesis> {
        info!("Starting knowledge synthesis for topic: {}", topic);
        
        // Search for relevant content
        let search_results = if let Some(scope) = scope_path {
            self.search_engine.search_by_scope(topic, scope, self.config.max_source_count).await?
        } else {
            self.search_engine.search_semantic(topic, self.config.max_source_count).await?
        };
        
        if search_results.len() < self.config.min_source_count {
            return Err(SynthesisError::InsufficientData(
                format!("Only {} sources found, need at least {}", 
                    search_results.len(), self.config.min_source_count)
            ).into());
        }
        ...
        for method in &self.config.synthesis_methods {
            let synthesized_content = self.synthesize_with_method(topic, &search_results, method).await?;
            synthesis_results.push(synthesized_content);
        }
        
        // Combine synthesis results
        let final_synthesis = self.combine_synthesis_results(topic, synthesis_results).await?;
        ...
        debug!("Knowledge synthesis completed for topic: {} (confidence: {:.2})", 
    ...
    async fn synthesize_with_method(
        ...
        match method {
            ...
            SynthesisMethod::DecisionTree => {
                self.decision_tree_synthesis(topic, results).await
            }
            SynthesisMethod::Hybrid => {
                self.hybrid_synthesis(topic, results).await
            }
        }
    }
    ...
    async fn semantic_clustering_synthesis(
        ...
        synthesis.push_str(&format!("This synthesis combines information from {} sources across {} semantic clusters.\n\n", 
            results.len(), clusters.len()));
    ...
    async fn decision_tree_synthesis(
        ...
        synthesis.push_str(&format!("**Root Decision:** {}\n\n", decision_tree.root_decision));
    ...
    async fn hybrid_synthesis(
        ...
        let cross_scope_insights = self.cross_scope_correlation_synthesis(topic, results).await?;
    ...
    async fn cluster_by_semantic_similarity(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<Vec<SemanticResult>>> {
    ...
    async fn extract_cluster_themes(&self, cluster: &[SemanticResult]) -> KnowledgeResult<Vec<String>> {
    ...
    async fn summarize_cluster_content(&self, cluster: &[SemanticResult]) -> KnowledgeResult<String> {
    ...
    async fn group_by_time_periods(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<(String, Vec<SemanticResult>)>> {
        ...
        for result in results {
            ...
            periods.entry(year).or_insert_with(Vec::new).push(result.clone());
        }
    ...
    async fn summarize_period_content(&self, results: &[SemanticResult]) -> KnowledgeResult<String> {
        let total_content: usize = results.iter().map(|r| r.content.len()).sum();
        Ok(format!("{} sources with {} total characters", results.len(), total_content))
    }
    
    async fn identify_temporal_trends(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<String>> {
    ...
    async fn group_by_scope(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<(String, Vec<SemanticResult>)>> {
        let mut scope_groups: HashMap<String, Vec<SemanticResult>> = HashMap::new();
        
        for result in results {
            let scope = result.metadata.scope_path.clone().unwrap_or_else(|| "unknown".to_string());
            scope_groups.entry(scope).or_insert_with(Vec::new).push(result.clone());
        }
    ...
    async fn summarize_scope_content(&self, results: &[SemanticResult]) -> KnowledgeResult<String> {
        ...
        Ok(format!("{} sources of types: {}", results.len(), content_types.join(", ")))
    }
    
    async fn extract_scope_insights(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<String>> {
        ...
        let content_types: Vec<ContentType> = results.iter().map(|r| r.metadata.source_type.clone()).collect();
        insights.push(format!("Content types: {:?}", content_types));
        
        Ok(insights)
    }
    
    async fn identify_cross_scope_patterns(&self, scope_groups: &[(String, Vec<SemanticResult>)]) -> KnowledgeResult<Vec<String>> {
        ...
        if scope_groups.len() > 1 {
            ...
            patterns.push(format!("Total sources across {} scopes: {}", scope_groups.len(), total_sources));
        }
        
        Ok(patterns)
    }
    
    async fn extract_content_patterns(&self, results: &[SemanticResult]) -> KnowledgeResult<HashMap<String, HashMap<String, usize>>> {
    ...
    async fn build_decision_tree(&self, _results: &[SemanticResult]) -> KnowledgeResult<DecisionTree> {
    ...
    async fn generate_integrated_conclusions(&self, results: &[SemanticResult]) -> KnowledgeResult<String> {
        let mut conclusions = String::new();
        
        let avg_relevance = results.iter().map(|r| r.relevance_score).sum::<f32>() / results.len() as f32;
        conclusions.push_str(&format!("Overall relevance score: {:.2}\n\n", avg_relevance));
    ...
    async fn extract_temporal_range(&self, results: &[SemanticResult]) -> KnowledgeResult<Option<(chrono::DateTime<chrono::Utc>, chrono::DateTime<chrono::Utc>)>> {
        if results.is_empty() {
            return Ok(None);
        }
        
        let earliest = results.iter().map(|r| r.metadata.created_at).min().unwrap();
        let latest = results.iter().map(|r| r.metadata.last_modified).max().unwrap();
        
        Ok(Some((earliest, latest)))
    }
    
    async fn extract_semantic_clusters(&self, results: &[SemanticResult]) -> KnowledgeResult<Vec<String>> {
        let clusters = self.cluster_by_semantic_similarity(results).await?;
        Ok(clusters.iter().map(|c| format!("cluster_{}", c.len())).collect())
    }
