...
use crate::{PatternUsage, Rhema, RhemaResult};
// PatternSubcommands will be defined in this module
use crate::file_ops;
use crate::scope::find_nearest_scope;

// Use the PatternSubcommands from commands module
use crate::commands::PatternSubcommands;
use colored::*;
...
pub fn run(rhema: &Rhema, subcommand: &PatternSubcommands) -> RhemaResult<()> {
    ...
    match subcommand {
        ...
        PatternSubcommands::Delete { id } => delete_pattern(scope, id),
    }
}
...
fn list_patterns(
    ...
    for pattern in patterns {
        ...
        println!(
            "📊 Usage: {}",
            format!("{:?}", pattern.usage).color(usage_color)
        );
        ...
        println!(
            "📅 Created: {}",
            pattern.created_at.format("%Y-%m-%d %H:%M")
        );
...
fn update_pattern(
    ...
    if name.is_some()
        ...
        || usage.is_some()
        || effectiveness.is_some()
        || examples.is_some()
        || anti_patterns.is_some()
    {
...
fn delete_pattern(scope: &rhema_core::scope::Scope, id: &str) -> RhemaResult<()> {
