...
use crate::{Rhema, RhemaResult, ConfigManager};
use crate::{GlobalConfig, RepositoryConfig, Config};
use rhema_config::{validation, backup, migration, ConfigHealth, ConfigHealthStatus, ConfigAuditLog, ConfigStats, ConfigIssueSeverity};
use colored::*;
...
fn validate_config(
    ...
    match config_type {
        "global" => {
            ...
            display_validation_report(&rhema_config::validation::ValidationReport {
                ...
                summary: rhema_config::validation::ValidationSummary {
                    ...
                    critical_issues: result.issues.iter().filter(|i| i.severity == rhema_config::ConfigIssueSeverity::Critical).count(),
                    error_issues: result.issues.iter().filter(|i| i.severity == rhema_config::ConfigIssueSeverity::Error).count(),
                    warning_issues: result.issues.iter().filter(|i| i.severity == rhema_config::ConfigIssueSeverity::Warning).count(),
                    info_issues: result.issues.iter().filter(|i| i.severity == rhema_config::ConfigIssueSeverity::Info).count(),
                },
...
fn get_config_value(
    ...
    let value = match config_type {
        "global" => {
            config_manager.global_config().get_value(key).cloned()
        }
        "repository" => {
            let _repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn display_restore_report(
    report: &rhema_config::backup::RestoreReport
) -> RhemaResult<()> {
...
fn display_migration_report(
    report: &rhema_config::migration::MigrationReport
) -> RhemaResult<()> {
...
fn display_config_health(
    health: &rhema_config::ConfigHealth
) -> RhemaResult<()> {
...
fn display_audit_log(
    ...
    for change in changes.iter().take(10) { // Show last 10 changes
        println!("    • {}: {} - {}", change.timestamp, change.action, change.details);
...
fn display_config_stats(
    stats: &rhema_config::ConfigStats
) -> RhemaResult<()> {
