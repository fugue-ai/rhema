...
use crate::file_ops;
use rhema_core::schema::{RhemaScope, Knowledge, Todos, Decisions, Patterns, Conventions};
use crate::{validate, health, stats, query, search, migrate, export_context, interactive_builder};
...
fn batch_validate_scope(_rhema: &Rhema, scope: &rhema_core::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
...
fn batch_migrate_scope(rhema: &Rhema, scope: &rhema_core::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn batch_migrate_scope_dry_run(_rhema: &Rhema, scope: &rhema_core::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
...
fn batch_export_scope(rhema: &Rhema, scope: &rhema_core::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn batch_import_scope(_rhema: &Rhema, scope: &rhema_core::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
...
fn batch_import_scope_dry_run(_rhema: &Rhema, scope: &rhema_core::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
...
fn batch_health_check_scope(rhema: &Rhema, scope: &rhema_core::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn execute_command_on_scope(rhema: &Rhema, scope: &rhema_core::scope::Scope, cmd: &BatchCommand) -> BatchItemResult {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn export_scope_data(_rhema: &Rhema, scope: &rhema_core::scope::Scope, all_data: &mut HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn import_scope_data(_rhema: &Rhema, scope: &rhema_core::scope::Scope, import_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
...
fn batch_validate_scope_comprehensive(_rhema: &Rhema, scope: &rhema_core::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn batch_health_check_scope_comprehensive(_rhema: &Rhema, scope: &rhema_core::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn batch_schema_check_scope(_rhema: &Rhema, scope: &rhema_core::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
...
fn batch_dependency_check_scope(rhema: &Rhema, scope: &rhema_core::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    if detailed {
        ...
        for other_scope in &all_scopes {
            ...
            if let Ok(other_def) = file_ops::read_yaml_file::<rhema_core::schema::RhemaScope>(&other_rhema_file) {
...
fn collect_scope_report_data(rhema: &Rhema, scope: &rhema_core::scope::Scope, report_type: &str, report_data: &mut HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
    ...
    // Read scope definition
    if let Some(rhema_file) = find_scope_file(scope_path) {
        if let Ok(scope_def) = file_ops::read_yaml_file::<rhema_core::schema::RhemaScope>(&rhema_file) {
    ...
    match report_type {
        ...
        "analytics" => {
            ...
            if knowledge_file.exists() {
                if let Ok(knowledge) = file_ops::read_yaml_file::<rhema_core::schema::Knowledge>(&knowledge_file) {
            ...
            if todos_file.exists() {
                if let Ok(todos) = file_ops::read_yaml_file::<rhema_core::schema::Todos>(&todos_file) {
            ...
            if decisions_file.exists() {
                if let Ok(decisions) = file_ops::read_yaml_file::<rhema_core::schema::Decisions>(&decisions_file) {
            ...
            if patterns_file.exists() {
                if let Ok(patterns) = file_ops::read_yaml_file::<rhema_core::schema::Patterns>(&patterns_file) {
