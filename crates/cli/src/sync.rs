...
use crate::{
    file_ops::read_yaml_file, scope::build_dependency_graph, Knowledge, KnowledgeEntry, Rhema,
    RhemaResult,
};
use chrono::Utc;
use colored::*;
        ...
        if scope.has_file("knowledge.yaml") {
            ...
            match read_yaml_file::<Knowledge>(&knowledge_file) {
                Ok(knowledge) => {
                    ...
                    println!(
                        "  📖 Found {} knowledge entries in {}",
                        entry_count, scope_path
                    );
                }
                Err(e) => {
...
fn sync_knowledge_across_scopes(
    ...
    println!(
        "{}",
        "🔄 Syncing knowledge across dependent scopes...".blue()
    );
    ...
    for scope in scopes {
        ...
        for dep_scope_path in dependencies {
            if let Some(dep_knowledge) = all_knowledge.get(&dep_scope_path) {
                ...
                let synced =
                    sync_knowledge_from_dependency(rhema, scope, &dep_scope_path, dep_knowledge)?;
                synced_count += synced;
            }
        }
    }
    ...
    println!(
        "  📤 Synced {} knowledge entries across scopes",
        synced_count
    );
...
fn sync_knowledge_from_dependency(
    _rhema: &Rhema,
    target_scope: &rhema_core::scope::Scope,
    dep_scope_path: &str,
...
fn should_sync_knowledge(
    entry: &KnowledgeEntry,
    _target_scope: &rhema_core::schema::RhemaScope,
) -> bool {
    ...
    if let Some(category) = &entry.category {
        ...
        if relevant_categories
            .iter()
            .any(|c| category.to_lowercase().contains(c))
        {
            return true;
        }
    }
    ...
    if let Some(tags) = &entry.tags {
        ...
        if tags.iter().any(|tag| {
            relevant_tags
                .iter()
                .any(|rt| tag.to_lowercase().contains(rt))
        }) {
            return true;
        }
    }
...
fn update_cross_references(
        ...
        if scope.has_file("knowledge.yaml") {
            ...
            for entry in &mut knowledge.entries {
                ...
                if let Some(updated_content) =
                    update_knowledge_references(entry, &scope_path, all_knowledge)
                {
    ...
    println!(
        "  🔄 Updated cross-references in {} scope(s)",
        updated_count
    );
...
fn resolve_knowledge_conflicts(rhema: &Rhema, conflicts: &[KnowledgeConflict]) -> RhemaResult<()> {
    if conflicts.is_empty() {
        return Ok(());
    }
    ...
    for conflict in conflicts {
        ...
        if let Some((best_scope, best_entry)) = sorted_entries.first() {
            println!(
                "    ✅ Using version from {} (confidence: {}, created: {})",
                best_scope,
...
fn update_knowledge_entry(
    ...
    let scope = scopes
        .iter()
        .find(|s| s.relative_path(rhema.repo_root()).unwrap_or_default() == scope_path);
