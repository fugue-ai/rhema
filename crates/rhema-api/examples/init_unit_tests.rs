...
use rhema_api::{init_run, Rhema, RhemaError, RhemaResult};
use std::fs;
...
"#,
        ...
        fs::write(
            self.repo_path.join("package.json"),
            r#"
{
  "name": "test-project",
  "version": "1.0.0",
...
"#,
    ...
    fn verify_rhema_yaml_content(&self, scope_path: &PathBuf) -> RhemaResult<()> {
        ...
        let name = rhema_scope.get("name").ok_or_else(|| {
            RhemaError::ValidationError("Missing 'name' field in rhema.yaml".to_string())
        })?;
        let scope_type = rhema_scope.get("scope_type").ok_or_else(|| {
            RhemaError::ValidationError("Missing 'scope_type' field in rhema.yaml".to_string())
        })?;
        let version = rhema_scope.get("version").ok_or_else(|| {
            RhemaError::ValidationError("Missing 'version' field in rhema.yaml".to_string())
        })?;
        ...
        if scope_type.as_str().unwrap_or("").is_empty() {
            return Err(RhemaError::ValidationError(
                "Scope type field is empty".to_string(),
            ));
        }
    ...
    fn verify_template_content(&self, scope_path: &PathBuf) -> RhemaResult<()> {
        ...
        if !knowledge_content.contains("Knowledge Base") {
            return Err(RhemaError::ValidationError(
                "Knowledge template missing expected content".to_string(),
            ));
        }
        if !knowledge_content.contains("entries: []") {
            return Err(RhemaError::ValidationError(
                "Knowledge template missing entries array".to_string(),
            ));
        }
        ...
        if !todos_content.contains("Todo Items") {
            return Err(RhemaError::ValidationError(
                "Todos template missing expected content".to_string(),
            ));
        }
        if !todos_content.contains("todos: []") {
            return Err(RhemaError::ValidationError(
                "Todos template missing todos array".to_string(),
            ));
        }
        ...
        if !decisions_content.contains("Decisions") {
            return Err(RhemaError::ValidationError(
                "Decisions template missing expected content".to_string(),
            ));
        }
        if !decisions_content.contains("decisions: []") {
            return Err(RhemaError::ValidationError(
                "Decisions template missing decisions array".to_string(),
            ));
        }
        ...
        if !patterns_content.contains("Patterns") {
            return Err(RhemaError::ValidationError(
                "Patterns template missing expected content".to_string(),
            ));
        }
        if !patterns_content.contains("patterns: []") {
            return Err(RhemaError::ValidationError(
                "Patterns template missing patterns array".to_string(),
            ));
        }
        ...
        if !conventions_content.contains("Conventions") {
            return Err(RhemaError::ValidationError(
                "Conventions template missing expected content".to_string(),
            ));
        }
        if !conventions_content.contains("conventions: []") {
            return Err(RhemaError::ValidationError(
                "Conventions template missing conventions array".to_string(),
            ));
        }
...
async fn test_basic_init_at_repo_root() -> RhemaResult<()> {
    ...
    if !scope_path.exists() {
        return Err(RhemaError::ValidationError(
            "Expected .rhema directory not created at repo root".to_string(),
        ));
    }
...
async fn test_basic_init_in_subdirectory() -> RhemaResult<()> {
    ...
    if !scope_path.exists() {
        return Err(RhemaError::ValidationError(
            "Expected .rhema directory not created in subdirectory".to_string(),
        ));
    }
...
async fn test_auto_configuration() -> RhemaResult<()> {
    ...
    if !scope_path.exists() {
        return Err(RhemaError::ValidationError(
            "Expected .rhema directory not created".to_string(),
        ));
    }
...
async fn test_error_handling_existing_files() -> RhemaResult<()> {
    ...
    match result {
        Ok(_) => {
            return Err(RhemaError::ValidationError(
                "Expected initialization to fail with existing files".to_string(),
            ))
        }
        Err(e) => {
            ...
            if !error_msg.contains("already exist") {
                return Err(RhemaError::ValidationError(format!(
                    "Expected error about existing files, got: {}",
                    error_msg
                )));
            }
        }
...
async fn test_custom_scope_type_and_name() -> RhemaResult<()> {
    ...
    if name != custom_scope_name {
        return Err(RhemaError::ValidationError(format!(
            "Expected name '{}', got '{}'",
            custom_scope_name, name
        )));
    }
    if scope_type != custom_scope_type {
        return Err(RhemaError::ValidationError(format!(
            "Expected scope_type '{}', got '{}'",
            custom_scope_type, scope_type
        )));
    }
...
async fn test_template_file_creation() -> RhemaResult<()> {
    ...
    if !knowledge_content.contains("categories:") {
        return Err(RhemaError::ValidationError(
            "Knowledge template missing categories section".to_string(),
        ));
    }
    ...
    if !todos_content.contains("Todo Items") {
        return Err(RhemaError::ValidationError(
            "Todos template missing expected header".to_string(),
        ));
    }
...
async fn test_protocol_info_generation() -> RhemaResult<()> {
    ...
    if version != "1.0.0" {
        return Err(RhemaError::ValidationError(format!(
            "Expected protocol version '1.0.0', got '{}'",
            version
        )));
    }

    // Verify concepts are present
    let concepts = protocol_info
        .get("concepts")
    ...
    if matches!(concepts, serde_yaml::Value::Null) {
        return Err(RhemaError::ValidationError(
            "Concepts should not be null".to_string(),
        ));
    }

    // Verify CQL examples are present
    let cql_examples = protocol_info.get("cql_examples").ok_or_else(|| {
        RhemaError::ValidationError("Protocol info missing cql_examples".to_string())
    })?;

    // Verify CQL examples are present and not null
    if matches!(cql_examples, serde_yaml::Value::Null) {
        return Err(RhemaError::ValidationError(
            "CQL examples should not be null".to_string(),
        ));
    }

    // Verify patterns are present
    let patterns = protocol_info
        .get("patterns")
    ...
    if matches!(patterns, serde_yaml::Value::Null) {
        return Err(RhemaError::ValidationError(
            "Patterns should not be null".to_string(),
        ));
    }
...
async fn test_edge_cases_and_errors() -> RhemaResult<()> {
    ...
    if !scope_path.exists() {
        return Err(RhemaError::ValidationError(
            "Expected .rhema directory not created with empty scope type".to_string(),
        ));
    }
    ...
    if !scope_path_long.exists() {
        return Err(RhemaError::ValidationError(
            "Expected .rhema directory not created with long name".to_string(),
        ));
    }
...
async fn test_rhema_integration() -> RhemaResult<()> {
    ...
    if scopes.is_empty() {
        return Err(RhemaError::ValidationError(
            "Rhema instance should discover the created scope".to_string(),
        ));
    }
    ...
    if knowledge.entries.len() != 0 {
        return Err(RhemaError::ValidationError(
            "New scope should have empty knowledge entries".to_string(),
        ));
    }
    ...
    if todos.todos.len() != 0 {
        return Err(RhemaError::ValidationError(
            "New scope should have empty todos".to_string(),
        ));
    }
...
async fn test_performance_and_concurrent_init() -> RhemaResult<()> {
    ...
    if duration > std::time::Duration::from_secs(5) {
        return Err(RhemaError::ValidationError(
            "Initialization took too long".to_string(),
        ));
    }
    ...
    for handle in handles {
        let result = handle
            .await
            .map_err(|e| RhemaError::ValidationError(format!("Join error: {}", e)))?;
        // All should fail due to existing files
        if result.is_ok() {
            return Err(RhemaError::ValidationError(
                "Concurrent initialization should fail due to existing files".to_string(),
            ));
        }
    }
...
mod additional_tests {
    ...
    async fn test_init_with_different_scope_types() -> RhemaResult<()> {
        ...
        for scope_type in &scope_types {
            ...
            let detected_scope_type = rhema_scope
                .get("scope_type")
                .and_then(|s| s.as_str())
                .ok_or_else(|| {
                    RhemaError::ValidationError("Missing scope_type field".to_string())
                })?;

            if detected_scope_type != *scope_type {
                return Err(RhemaError::ValidationError(format!(
                    "Expected scope_type '{}', got '{}'",
                    scope_type, detected_scope_type
                )));
            }
        }
