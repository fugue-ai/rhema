...
pub struct OfficialRhemaMcpServer {
    ...
    connections: Arc<RwLock<HashMap<String, std::time::Instant>>>,
}
...
impl OfficialRhemaMcpServer {
    ...
    pub async fn track_connection(&self, client_id: String) -> RhemaResult<()> {
        let mut connections = self.connections.write().await;
        connections.insert(client_id, std::time::Instant::now());
        Ok(())
    }
    ...
    pub async fn update_client_activity(&self, client_id: &str) -> RhemaResult<()> {
        ...
        if let Some(connection) = connections.get_mut(client_id) {
            *connection = std::time::Instant::now();
        }
        Ok(())
    }
    ...
    async fn get_memory_usage(&self) -> crate::mcp::MemoryUsage {
        // Get system memory info using sysinfo
        let mut sys = sysinfo::System::new_all();
        sys.refresh_memory();

        let total = sys.total_memory() * 1024; // Convert KB to bytes
        let used = (sys.total_memory() - sys.available_memory()) * 1024; // Convert KB to bytes

        // Get cache size
        let cache_size = self.cache_manager.get_size().await;
