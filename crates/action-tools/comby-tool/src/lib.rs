...
impl CombyTool {
    ...
    async fn execute_comby_on_file(&self, pattern: &str, rewrite: &str, file_path: &str) -> ActionResult<String> {
        ...
        let output = tokio::process::Command::new("comby")
            ...
            .output()
            .await
            .map_err(|e| ActionError::ToolExecution { tool: "comby".to_string(), message: format!("Failed to execute comby: {}", e) })?;
        ...
        if output.status.success() {
            ...
            let stderr = String::from_utf8_lossy(&output.stderr);
            Err(ActionError::ToolExecution { tool: "comby".to_string(), message: format!("Comby failed for {}: {}", file_path, stderr) })
        }
    }
} 