...
impl ValidationTool for CargoTool {
    async fn validate(&self, intent: &ActionIntent) -> ActionResult<ToolResult> {
        ...
        Ok(ToolResult {
            ...
            changes: all_changes,
            output: format!(
                "Cargo validation completed for {} projects",
                cargo_files.len()
            ),
            errors: all_errors,
    ...
    async fn execute(&self, intent: &ActionIntent) -> ActionResult<ToolResult> {
        ...
        if cargo_files.is_empty() {
            return Err(ActionError::Validation(
                "No Cargo.toml files found for transformation".to_string(),
            ));
        }
        ...
        Ok(ToolResult {
            ...
            changes: all_changes,
            output: format!(
                "Cargo transformations completed for {} projects",
                cargo_files.len()
            ),
            errors: all_errors,
            warnings: all_warnings,
...
impl CargoTool {
    ...
    ) -> Result<PackageInfo, ActionError> {
    ...
    async fn run_cargo_commands_with_workspace(
        &self,
        cargo_file: &str,
        config: &CargoConfig,
                ...
                match config.workspace_mode {
                    WorkspaceMode::RootOnly => {
                        ...
                        for command in &config.commands {
                                ...
                                .await
                            {
                                ...
                                Err(e) => {
                                    error!(
                                        "Failed to execute {:?} for workspace root: {}",
                                        command, e
                                    );
                    ...
                    WorkspaceMode::SelectedMembers => {
                        ...
                        let selected_members =
                            self.get_selected_members(&workspace.members, config);
    ...
    async fn run_transformation_commands_with_workspace(
        &self,
        cargo_file: &str,
        config: &CargoConfig,
    ) -> ActionResult<Vec<CargoResult>> {
                    ...
                    WorkspaceMode::RootOnly => {
                        ...
                        if config.commands.contains(&CargoCommand::Clippy) {
                            match self.execute_cargo_clippy_fix(project_dir, config).await {
                                ...
                                Err(e) => {
                                    error!(
                                        "Failed to execute clippy fix for workspace root: {}",
                                        e
                                    );
                                    results.push(CargoResult {
    ...
    async fn run_transformation_commands(
        &self,
        cargo_file: &str,
        config: &CargoConfig,
    ) -> ActionResult<Vec<CargoResult>> {
    ...
    async fn execute_cargo_clippy_fix(
        &self,
        project_dir: &std::path::Path,
        config: &CargoConfig,
    ) -> ActionResult<CargoResult> {
