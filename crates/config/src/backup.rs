...
impl BackupManager {
    ...
    pub fn backup_config<T: Config>(&mut self, config: &T, context: &str) -> RhemaResult<BackupRecord> {
        ...
        // Write backup file
        fs::write(&backup_path, &final_content)
            .map_err(|e| ConfigError::IoError(e))?;
        ...
        let history = self.backup_history.entry(PathBuf::from(context)).or_insert_with(Vec::new);
        ...
        match self.backup_format {
            ...
            BackupFormat::TOML => {
                let content = toml::to_string_pretty(config)
                    .map_err(|e| ConfigError::SerializationError(serde_json::Error::io(std::io::Error::new(std::io::ErrorKind::InvalidData, e.to_string()))))?;
                Ok(content.into_bytes())
            }
            BackupFormat::Binary => {
    ...
    fn find_backup_record(&self, config_type: &str, backup_id: &str) -> RhemaResult<&BackupRecord> {
        ...
        Err(ConfigError::BackupFailed(
            format!("Backup with ID '{}' not found for config type '{}'", backup_id, config_type)
        ).into())
    }

    /// Deserialize configuration
    fn deserialize_config<T: Config>(&self, content: &[u8], format: &BackupFormat) -> RhemaResult<T> {
