...
use crate::{ConfigAuditLog, ConfigHealth, ConfigStats, ConfigEnvironment, Config, CURRENT_CONFIG_VERSION};
...
impl Config for GlobalConfig {
    ...
    fn validate_config(&self) -> rhema_core::RhemaResult<()> {
        ...
        if self.version.is_empty() {
            return Err(rhema_core::RhemaError::ConfigError("Version cannot be empty".to_string()));
        }
        Ok(())
    }
