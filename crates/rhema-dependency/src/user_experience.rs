...
impl DependencyDashboard {
    ...
    pub async fn update_data(&self, dependencies: &[DependencyConfig]) -> Result<()> {
        ...
        data.healthy_dependencies = dependencies
            .iter()
            .filter(|d| d.health_status == HealthStatus::Healthy)
            .count();
        data.unhealthy_dependencies = dependencies
            .iter()
            .filter(|d| d.health_status != HealthStatus::Healthy)
            .count();
        data.critical_dependencies = dependencies
            .iter()
            .filter(|d| d.health_status == HealthStatus::Down)
            .count();
        ...
        for dep in dependencies {
            *data
                .dependencies_by_health
                .entry(dep.health_status.clone())
                .or_insert(0) += 1;
        }
...
impl DependencyAlertSystem {
    ...
    async fn evaluate_condition(
        ...
        match condition {
            AlertCondition::HealthStatus(status) => Ok(dependency.health_status == *status),
    ...
    fn find_matched_fields(
        ...
        for query_term in query_terms {
            ...
            if dependency
                .operations
                .iter()
                .any(|op| op.to_lowercase().contains(query_term))
            {
