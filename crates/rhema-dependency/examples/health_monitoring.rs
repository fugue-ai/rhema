use rhema_dependency::{
    DependencyManager, DependencyType, HealthMetrics, HealthStatus, ImpactScore,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    manager
        .update_health_metrics(
            "postgres-db",
            ImpactScore::new(0.6, 0.5, 0.4, 0.3, 0.2, 0.1)?,
        )
        .await?;
    manager
        .update_health_status("postgres-db", HealthStatus::Degraded)
        .await?;
    print_health_summary(&manager).await?;
    sleep(Duration::from_secs(2)).await;
    ...
    manager
        .update_health_status("user-api", HealthStatus::Down)
        .await?;
    manager
        .update_health_metrics("user-api", ImpactScore::new(0.9, 0.8, 0.7, 0.6, 0.5, 0.4)?)
        .await?;
    print_health_summary(&manager).await?;
    sleep(Duration::from_secs(2)).await;
    ...
    manager
        .update_health_status("redis-cache", HealthStatus::Unhealthy)
        .await?;
    manager
        .update_health_metrics(
            "redis-cache",
            ImpactScore::new(0.7, 0.6, 0.5, 0.4, 0.3, 0.2)?,
        )
        .await?;
    print_health_summary(&manager).await?;
    sleep(Duration::from_secs(2)).await;
    ...
    manager
        .update_health_status("payment-gateway", HealthStatus::Degraded)
        .await?;
    manager
        .update_health_metrics(
            "payment-gateway",
            ImpactScore::new(0.8, 0.7, 0.6, 0.5, 0.4, 0.3)?,
        )
        .await?;
    print_health_summary(&manager).await?;
    sleep(Duration::from_secs(2)).await;
    ...
    if !unhealthy_dependencies.is_empty() {
        ...
        for (id, status) in unhealthy_dependencies {
            println!(
                "  - {}: {:?} (Health Score: {:.2})",
                id, status.status, status.health_score
            );
        }
        println!();
    ...
    println!(
        ...
        final_report.critical_dependencies.len()
    );
    println!("Graph Statistics:");
    println!(
        "  Total Dependencies: {}",
        final_report.graph_statistics.total_nodes
    );
    println!(
        "  Total Relationships: {}",
        final_report.graph_statistics.total_edges
    );
    println!("  Healthy: {}", final_report.graph_statistics.healthy_count);
    println!(
        "  Degraded: {}",
        final_report.graph_statistics.degraded_count
    );
    println!(
        "  Unhealthy: {}",
        final_report.graph_statistics.unhealthy_count
    );
...
async fn update_health_statuses(
    manager: &mut DependencyManager,
    status: HealthStatus,
) -> Result<(), Box<dyn std::error::Error>> {
    for dependency_id in ["postgres-db", "user-api", "redis-cache", "payment-gateway"] {
        manager
            .update_health_status(dependency_id, status.clone())
            .await?;
    }
    Ok(())
}

async fn print_health_summary(
