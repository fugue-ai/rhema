...
impl LockManager {
    ...
    pub fn has_agent_locks(&self, agent_id: &str) -> bool {
        self.locks
            .values()
            .any(|lock_agent_id| lock_agent_id.as_deref() == Some(agent_id))
    }
    ...
    pub async fn acquire_lock(&mut self, scope_path: &str, agent_id: &str) -> RhemaResult<bool> {
        ...
        // Acquire the lock
        self.locks
            .insert(scope_path.to_string(), Some(agent_id.to_string()));
    ...
    pub async fn release_lock(&mut self, scope_path: &str, agent_id: &str) -> RhemaResult<()> {
        ...
        match current_agent {
            ...
            Some(current_agent_id) => Err(LockError::LockNotHeld(current_agent_id.clone()).into()),
            None => Err(LockError::LockNotHeld("no agent".to_string()).into()),
        }
    }
    ...
    pub fn is_locked(&self, scope_path: &str) -> bool {
        self.locks
            .get(scope_path)
            .map_or(false, |agent_id| agent_id.is_some())
    }
    ...
    pub fn get_lock_holder(&self, scope_path: &str) -> Option<&String> {
        self.locks
            .get(scope_path)
            .and_then(|agent_id| agent_id.as_ref())
    }
        ...
        for scope_path in expired_locks {
            if let Some(agent_id) = self.locks.get(&scope_path).and_then(|id| id.as_ref()) {
                ...
                self.record_lock_event(
                    &scope_path,
                    &agent_id,
                    LockEventType::Timeout,
                    Some("Lock expired".to_string()),
                );
            }
        }
    ...
    /// Extend lock timeout
    pub async fn extend_lock_timeout(
        &mut self,
        scope_path: &str,
        agent_id: &str,
    ) -> RhemaResult<()> {
    ...
    pub fn get_lock_statistics(&self) -> LockStatistics {
        ...
        stats.locked_scopes = self
            .locks
            .values()
            .filter(|agent_id| agent_id.is_some())
            .count();
    ...
    pub fn cleanup_history(&mut self, max_history_size: usize) {
        if self.lock_history.len() > max_history_size {
            self.lock_history
                .drain(0..self.lock_history.len() - max_history_size);
        }
    }
    ...
    pub fn validate_consistency(&self) -> RhemaResult<()> {
            ...
            match agent_id {
                Some(agent_id) => {
                    ...
                    if !self.lock_info.contains_key(scope_path) {
                        return Err(LockError::LockConsistencyViolation(format!(
                            "Lock info missing for scope {}",
                            scope_path
                        ))
                        .into());
                    }
                ...
                None => {
                    ...
                    if self.lock_info.contains_key(scope_path) {
                        return Err(LockError::LockConsistencyViolation(format!(
                            "Lock info exists for unlocked scope {}",
                            scope_path
                        ))
                        .into());
                    }
        ...
        for scope_path in self.lock_info.keys() {
            if !self.locks.contains_key(scope_path) {
                return Err(LockError::LockConsistencyViolation(format!(
                    "Lock info exists for non-existent scope {}",
                    scope_path
                ))
                .into());
            }
...
impl std::fmt::Display for LockStatistics {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Total: {}, Locked: {}, Unlocked: {}, Max per agent: {}, Agents with locks: {}",
            self.total_scopes,
            self.locked_scopes,
            self.unlocked_scopes,
            self.max_locks_per_agent,
            self.agents_with_locks
        )
    }
}
