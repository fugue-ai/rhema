...
impl ActionValidator {
    ...
    pub async fn validate_action(&self, intent: &SchemaActionIntent) -> Result<ValidationResult> {
        ...
        match intent.action_type {
            ...
            ActionType::Performance => {
                self.validate_performance_action(
                    &shared_intent,
                    &mut validation_errors,
                    &mut validation_warnings,
                )
            ...
            ActionType::Configuration => {
                self.validate_configuration_action(
                    &shared_intent,
                    &mut validation_errors,
                    &mut validation_warnings,
                )
            ...
            ActionType::Dependency => {
                self.validate_dependency_action(
                    &shared_intent,
                    &mut validation_errors,
                    &mut validation_warnings,
                )
            ...
            ActionType::Migration => {
                self.validate_migration_action(
                    &shared_intent,
                    &mut validation_errors,
                    &mut validation_warnings,
                )
        ...
        if success {
            ...
            warn!(
                "Action validation completed with {} errors in {:?}",
                errors_count, duration
            );
        }
    ...
    async fn validate_refactor_action(
        ...
        let tool_result = self
            .tool_registry
            .execute_validation("typescript", intent)
            .await
            .map_err(|e| anyhow::anyhow!("TypeScript validation failed: {:?}", e))?;
    ...
    async fn validate_security_action(
        ...
        let security_result = self
            .tool_registry
            .execute_safety_check("security_scanning", intent)
            .await
            .map_err(|e| anyhow::anyhow!("Security scanning failed: {:?}", e))?;
    ...
    async fn validate_performance_action(
        ...
        let coverage_result = self
            .tool_registry
            .execute_safety_check("test_coverage", intent)
            .await
            .map_err(|e| anyhow::anyhow!("Test coverage check failed: {:?}", e))?;
