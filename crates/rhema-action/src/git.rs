...
impl ActionGitIntegration {
    ...
    pub fn get_current_branch(&self) -> ActionResult<String> {
        if let Some(repo) = &self.repository {
            let head = repo
                .head()
                .map_err(|e| ActionError::git("get_head", format!("Failed to get HEAD: {}", e)))?;

            let branch_name = head
                .shorthand()
                .ok_or_else(|| ActionError::git("get_branch", "Failed to get branch name"))?;
    ...
    pub fn commit_changes(&self, intent: &ActionIntent, message: &str) -> ActionResult<String> {
        if let Some(repo) = &self.repository {
            let signature =
                Signature::now("Rhema Action Protocol", "action@rhema.dev").map_err(|e| {
                    ActionError::git(
                        "create_signature",
            ...
            let commit_id = repo
                ...
                .map_err(|e| ActionError::git("commit", format!("Failed to commit: {}", e)))?;
    ...
    pub fn push_changes(&self, remote_name: &str) -> ActionResult<()> {
        if let Some(repo) = &self.repository {
            let mut remote = repo.find_remote(remote_name).map_err(|e| {
                ActionError::git(
                    "find_remote",
                    format!("Failed to find remote {}: {}", remote_name, e),
                )
            })?;
    ...
    pub fn create_pull_request(
        ...
        if let Some(repo) = &self.repository {
            ...
            let pr_url = format!("https://github.com/example/repo/pull/{}", intent.id);
    ...
    pub fn get_file_diff(&self, file_path: &str) -> ActionResult<String> {
        if let Some(repo) = &self.repository {
            ...
            let head_tree = head_commit
                .tree()
                .map_err(|e| ActionError::git("get_tree", format!("Failed to get tree: {}", e)))?;
            ...
            .map_err(|e| ActionError::git("print_diff", format!("Failed to print diff: {}", e)))?;
    ...
    pub async fn create_feature_branch(
        &self,
        intent: &ActionIntent,
    ) -> ActionResult<GitOperationResult> {
    ...
    pub async fn commit_staged_changes(
        ...
        if let Some(repo) = &self.repository {
            ...
            let tree_id = repo
                .index()
                .map_err(|e| ActionError::git("get_index", format!("Failed to get index: {}", e)))?
                .write_tree()
                .map_err(|e| {
                    ActionError::git("write_tree", format!("Failed to write tree: {}", e))
                })?;
    ...
    pub async fn get_detailed_diff(&self, files: &[String]) -> ActionResult<String> {
        if let Some(repo) = &self.repository {
            ...
            let diff = repo
                ...
                .map_err(|e| ActionError::git("diff", format!("Failed to create diff: {}", e)))?;
        ...
        if let Some(repo) = &self.repository {
            ...
            for branch_result in branches {
                let (branch, _) = branch_result.map_err(|e| {
                    ActionError::git(
                        "iterate_branches",
                        format!("Failed to iterate branches: {}", e),
                    )
                })?;
                ...
                let ahead_count = repo
                    .graph_ahead_behind(commit.id(), repo.head().unwrap().target().unwrap())
                    .map_err(|e| {
                        ActionError::git(
                            "graph_ahead_behind",
                            format!("Failed to calculate ahead/behind: {}", e),
                        )
                    })?
                    .0;
    ...
    pub async fn push_to_remote(
        ...
        if let Some(repo) = &self.repository {
            ...
            remote
                .push(&[&refspec], None)
                .map_err(|e| ActionError::git("push", format!("Failed to push: {}", e)))?;
