...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::write(&shared_file, "pub fn shared_function() {\n    println!(\"Feature branch version\");\n}")?;
    ...
    repo.commit(Some("refs/heads/feature/conflict-test"), &signature, &signature, 
        "Add shared function in feature branch", &tree, &[&current_commit])?;
    ...
    let tree = repo.find_tree(tree_id)?;
    let current_commit = repo.find_commit(repo.head()?.target().unwrap())?;
    repo.commit(Some("refs/heads/develop"), &signature, &signature, 
        "Modify shared function in develop", &tree, &[&current_commit])?;
