...
pub use automation::{
    default_automation_config, AutomationConfig, AutomationTask, GitAutomationManager,
};
pub use branch::{BranchContext, BranchContextManager, MergeStrategy, ValidationStatus};
pub use history::{ContextBlame, ContextEvolution, ContextHistoryManager, ContextVersion};
pub use hooks::{default_hook_config, HookConfig, HookManager, HookResult, HookType};
pub use monitoring::{default_monitoring_config, GitMonitoringManager, MonitoringConfig};
pub use security::{default_security_config, SecurityConfig, SecurityManager};
pub use workflow::{
    default_git_flow_config, FlowBranchType, WorkflowConfig, WorkflowManager, WorkflowType,
};
...
impl AdvancedGitIntegration {
    ...
    pub fn initialize(&mut self) -> RhemaResult<()> {
        ...
        println!(
            "Advanced Git integration initialized at {}",
    ...
    pub fn create_feature_branch(
        ...
        feature_name: &str,
        _base_branch: &str,
    ) -> RhemaResult<workflow::FeatureBranch> {
        let repo = Repository::open(&self.repo_path)?;
    ...
    pub fn finish_feature_branch(
        &mut self,
        feature_name: &str,
    ) -> RhemaResult<workflow::FeatureResult> {
        let repo = Repository::open(&self.repo_path)?;
        let workflow_manager = WorkflowManager::new(repo, self.config.workflow.clone());
    ...
    pub fn analyze_pull_request(
        &self,
        pr_number: u64,
    ) -> RhemaResult<workflow::PullRequestAnalysis> {
        let repo = Repository::open(&self.repo_path)?;
        let workflow_manager = WorkflowManager::new(repo, self.config.workflow.clone());
    ...
    pub fn create_context_version(
        &mut self,
        version: &str,
        version_type: history::VersionType,
        description: &str,
    ...
    pub fn security(&self) -> security::SecurityManager {
        let repo = Repository::open(&self.repo_path).expect("Failed to open repository");
        security::SecurityManager::new(repo, self.config.security.clone())
    ...
    pub fn validate_commit_security(
        &self,
        commit_hash: &str,
    ) -> RhemaResult<security::SecurityValidationResult> {
        let repo = Repository::open(&self.repo_path)?;
    ...
    pub fn monitoring(&self) -> monitoring::GitMonitoringManager {
        let repo = Repository::open(&self.repo_path).expect("Failed to open repository");
        monitoring::GitMonitoringManager::new(repo, self.config.monitoring.clone())
            .expect("Failed to create monitoring manager")
    }
    ...
    pub fn merge_branch_context(
        ...
        source_branch: &str,
        target_branch: &str,
    ) -> RhemaResult<branch::MergeResult> {
        let repo = Repository::open(&self.repo_path)?;
        let mut branch_manager = BranchContextManager::new(repo);
    ...
    pub fn check_context_conflicts(
        ...
        source_branch: &str,
        target_branch: &str,
    ) -> RhemaResult<Vec<branch::ContextConflict>> {
        let repo = Repository::open(&self.repo_path)?;
    ...
    pub fn resolve_context_conflicts(
        &mut self,
        conflicts: Vec<branch::ContextConflict>,
    ) -> RhemaResult<()> {
        let repo = Repository::open(&self.repo_path)?;
        let mut branch_manager = BranchContextManager::new(repo);
    ...
    ) -> RhemaResult<history::EvolutionReport> {
        let repo = Repository::open(&self.repo_path)?;
        let mut history_manager = ContextHistoryManager::new(repo);
    ...
    pub fn get_integration_status(&self) -> RhemaResult<IntegrationStatus> {
        ...
        let automation_manager = GitAutomationManager::new(
            Repository::open(&self.repo_path)?,
    ...
    fn save_state(&self) -> RhemaResult<()> {
        let state_file = self.repo_path.join(".rhema").join("integration-state.yaml");
