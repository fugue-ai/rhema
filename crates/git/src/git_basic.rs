...
pub fn is_tracked(repo: &Repository, path: &Path) -> Result<bool, RhemaError> {
    ...
    let relative_path = path.strip_prefix(repo_path).map_err(|_| {
        RhemaError::GitError(git2::Error::from_str("Path not in repository"))
    })?;
    
    let status = repo.status_file(relative_path).map_err(|e| RhemaError::GitError(e))?;
    Ok(!status.is_empty())
}
...
pub fn get_file_status(repo: &Repository, path: &Path) -> Result<git2::Status, RhemaError> {
    ...
    repo.status_file(relative_path).map_err(|e| RhemaError::GitError(e))
}
...
pub fn get_changed_files(repo: &Repository) -> Result<Vec<PathBuf>, RhemaError> {
    ...
    let statuses = repo.statuses(Some(&mut options)).map_err(|e| RhemaError::GitError(e))?;
    let repo_path = repo.path().parent().ok_or_else(|| {
        RhemaError::GitError(git2::Error::from_str("Invalid repository path"))
    })?;
...
pub fn stage_file(repo: &Repository, path: &Path) -> Result<(), RhemaError> {
    ...
    index.add_path(relative_path).map_err(|e| RhemaError::GitError(e))?;
...
pub fn commit_changes(repo: &Repository, message: &str) -> Result<(), RhemaError> {
    ...
    let tree_id = repo.index().map_err(|e| RhemaError::GitError(e))?.write_tree().map_err(|e| RhemaError::GitError(e))?;
    let tree = repo.find_tree(tree_id).map_err(|e| RhemaError::GitError(e))?;
