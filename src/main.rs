...
use rhema::{Rhema, RhemaResult, TodoSubcommands, InsightSubcommands, PatternSubcommands, DecisionSubcommands, GitSubcommands, PerformanceSubcommands};

// Import command modules
use rhema::commands;
use rhema::commands::context_rules::ContextRulesSubcommands;
use rhema::commands::workflow::WorkflowSubcommands;
use rhema::commands::template::TemplateSubcommands;
use rhema::commands::prompt::PromptSubcommands;
...
fn main() -> RhemaResult<()> {
    ...
    match cli.command {
        Commands::Init { scope_type, scope_name, auto_config } => {
            commands::init::run(&rhema, scope_type.as_deref(), scope_name.as_deref(), auto_config)
        }
        Commands::Scopes => {
            commands::scopes::run(&rhema)
        }
        Commands::Scope { path } => {
            commands::scopes::show_scope(&rhema, path.as_deref())
        }
        Commands::Tree => {
            commands::scopes::show_tree(&rhema)
        }
        Commands::Show { file, scope } => {
            commands::show::run(&rhema, &file, scope.as_deref())
        }
        Commands::Query { query, stats, format, provenance, field_provenance } => {
            if field_provenance {
                commands::query::run_with_field_provenance(&rhema, &query)
            } else if provenance {
                commands::query::run_with_provenance(&rhema, &query)
            } else if stats {
                commands::query::run_with_stats(&rhema, &query)
            } else if format != "yaml" {
                commands::query::run_formatted(&rhema, &query, format.as_str())
            } else {
                commands::query::run(&rhema, &query)
            }
        }
        Commands::Search { term, in_file, regex } => {
            commands::search::run(&rhema, &term, in_file.as_deref(), regex)
        }
        Commands::Validate { recursive, json_schema, migrate } => {
            commands::validate::run(&rhema, recursive, json_schema, migrate)
        }
        Commands::Migrate { recursive, dry_run } => {
            commands::migrate::run(&rhema, recursive, dry_run)
        }
        Commands::Schema { template_type, output_file } => {
            commands::schema::run(&rhema, &template_type, output_file.as_deref())
        }
        Commands::Health { scope } => {
            commands::health::run(&rhema, scope.as_deref())
        }
        Commands::Stats => {
            commands::stats::run(&rhema)
        }
        Commands::Todo { subcommand } => {
            commands::todo::run(&rhema, &subcommand)
        }
        Commands::Insight { subcommand } => {
            commands::insight::run(&rhema, &subcommand)
        }
        Commands::Pattern { subcommand } => {
            commands::pattern::run(&rhema, &subcommand)
        }
        Commands::Decision { subcommand } => {
            commands::decision::run(&rhema, &subcommand)
        }
        Commands::Dependencies => {
            commands::dependencies::run(&rhema)
        }
        Commands::Impact { file } => {
            commands::impact::run(&rhema, &file)
        }
        Commands::SyncKnowledge => {
            commands::sync::run(&rhema)
        }
        Commands::Git { subcommand } => {
            commands::git::run(&rhema, &subcommand)
        }
        Commands::ExportContext { format, output_file, scope_filter, include_protocol, include_knowledge, include_todos, include_decisions, include_patterns, include_conventions, summarize, ai_agent_format } => {
            commands::export_context::run(&rhema, &format, output_file.as_deref(), scope_filter.as_deref(), include_protocol, include_knowledge, include_todos, include_decisions, include_patterns, include_conventions, summarize, ai_agent_format)
        }
        Commands::Primer { scope_name, output_dir, template_type, include_examples, validate } => {
            commands::primer::run(&rhema, scope_name.as_deref(), output_dir.as_deref(), template_type.as_deref(), include_examples, validate)
        }
        Commands::GenerateReadme { scope_name, output_file, template, include_context, seo_optimized, custom_sections } => {
            ...
            commands::generate_readme::run(&rhema, scope_name.as_deref(), output_file.as_deref(), template.as_deref(), include_context, seo_optimized, custom_sections_vec)
        }
        Commands::BootstrapContext { use_case, output_format, output_dir, scope_filter, include_all, optimize_for_ai, create_primer, create_readme } => {
            commands::bootstrap_context::run(&rhema, &use_case, &output_format, output_dir.as_deref(), scope_filter.as_deref(), include_all, optimize_for_ai, create_primer, create_readme)
        }
        Commands::Daemon { args } => {
            tokio::runtime::Runtime::new()?.block_on(commands::daemon::execute_daemon(args))
        }
        Commands::Performance { subcommand } => {
            tokio::runtime::Runtime::new()?.block_on(commands::performance::run_performance_command(&rhema, &subcommand))
        }
        Commands::Config { subcommand } => {
            commands::config::run(&rhema, &subcommand)
        }
        Commands::Prompt { subcommand } => {
            commands::prompt::run(&rhema, &subcommand)
        }
        Commands::ContextRules { subcommand } => {
            commands::context_rules::run(&rhema, &subcommand)
        }
        Commands::Workflow { subcommand } => {
            commands::workflow::run(&rhema, &subcommand)
        }
        Commands::Template { subcommand } => {
            commands::template::run(&rhema, &subcommand)
        }
        Commands::Interactive { config, no_auto_complete, no_syntax_highlighting, no_context_aware } => {
            commands::interactive::run_interactive_with_config(
                rhema,
                config.as_deref(),
                no_auto_complete,
