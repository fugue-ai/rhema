...
use crate::{RhemaResult, scope::Scope, schema::*};
...
impl ContextProvider {
    ...
    pub async fn search_regex(&self, pattern: &str, file_filter: Option<&str>) -> RhemaResult<Vec<QueryResult>> {
        crate::query::search_context_regex(&self.repo_root, pattern, file_filter)
    }
    ...
    async fn load_scopes(&self) -> RhemaResult<()> {
        ...
        let mut scopes_lock = self.scopes.write().await;
        *scopes_lock = scopes;
