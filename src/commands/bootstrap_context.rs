...
pub fn run(
    ...
    let filtered_scopes = if let Some(filter) = scope_filter {
        scopes.into_iter()
            .filter(|scope| scope.definition.name.contains(filter) || scope.definition.scope_type.contains(filter))
            .collect()
    } else {
        scopes
    };
    ...
    let rhema_scopes: Vec<RhemaScope> = filtered_scopes.iter()
        .map(|scope| scope.definition.clone())
        .collect();
    
    let bootstrap_content = generate_bootstrap_content(
        rhema,
        &rhema_scopes,
        use_case,
        include_all,
...
fn generate_bootstrap_content(
    ...
    let metadata = BootstrapMetadata {
        ...
        optimization: if optimize_for_ai { "ai_optimized".to_string() } else { "standard".to_string() },
    };
...
fn create_scope_summaries(
    ...
    for scope in scopes {
        ...
        let dependencies = scope.dependencies.as_ref().map(|deps| {
...
fn determine_context_relevance(_scope: &RhemaScope, use_case: &str) -> String {
    match use_case {
        "code_review" => "High - Contains patterns, conventions, and architectural decisions".to_string(),
        "feature_development" => "High - Provides implementation patterns and integration context".to_string(),
        ...
        "onboarding" => "High - Essential for understanding project structure and decisions".to_string(),
...
fn format_bootstrap_markdown(content: &BootstrapContent) -> String {
    ...
    md.push_str(&format!("# Rhema Context Bootstrap - {}\n\n", content.use_case.name));
    ...
    if let Some(ref ai_instructions) = content.ai_instructions {
        ...
        md.push_str(&format!("**Context Understanding:** {}\n\n", ai_instructions.context_understanding));
...
fn format_bootstrap_text(content: &BootstrapContent) -> String {
    ...
    text.push_str(&format!("Rhema CONTEXT BOOTSTRAP - {}\n", content.use_case.name.to_uppercase()));
    ...
    text.push_str(&format!("Optimization: {}\n\n", content.metadata.optimization));
    ...
    if let Some(ref ai_instructions) = content.ai_instructions {
        ...
        text.push_str("----------------------\n");
        text.push_str(&format!("Context Understanding: {}\n\n", ai_instructions.context_understanding));
