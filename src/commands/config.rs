...
use crate::config::{ConfigManager, GlobalConfig, RepositoryConfig, Config};
use colored::*;
...
pub fn run(_rhema: &Rhema, subcommand: &ConfigSubcommands) -> RhemaResult<()> {
    ...
    match subcommand {
        ConfigSubcommands::Show { config_type, path, format } => {
            show_config(&mut config_manager, config_type, path.as_deref(), format)
        }
        ConfigSubcommands::Edit { config_type, path, editor } => {
            edit_config(&mut config_manager, config_type, path.as_deref(), editor.clone())
        }
        ConfigSubcommands::Validate { config_type, path, fix } => {
            validate_config(&mut config_manager, config_type, path.as_deref(), *fix)
        }
        ConfigSubcommands::Backup { config_type, path, output } => {
            backup_config(&mut config_manager, config_type, path.as_deref(), output.as_deref())
        }
        ConfigSubcommands::Restore { config_type, path, backup_file } => {
            restore_config(&mut config_manager, config_type, path.as_deref(), backup_file)
        }
        ConfigSubcommands::Migrate { config_type, path, dry_run } => {
            migrate_config(&mut config_manager, config_type, path.as_deref(), *dry_run)
        }
        ConfigSubcommands::Export { config_type, path, format, output } => {
            export_config(&mut config_manager, config_type, path.as_deref(), format, output.as_deref())
        }
        ConfigSubcommands::Import { config_type, path, format, input } => {
            import_config(&mut config_manager, config_type, path.as_deref(), format, input)
        }
        ConfigSubcommands::Set { config_type, path, key, value } => {
            set_config_value(&mut config_manager, config_type, path.as_deref(), key, value)
        }
        ConfigSubcommands::Get { config_type, path, key } => {
            get_config_value(&mut config_manager, config_type, path.as_deref(), key)
        }
        ConfigSubcommands::Reset { config_type, path, confirm } => {
        ...
        ConfigSubcommands::Audit { config_type, path, since } => {
            audit_config(&mut config_manager, config_type, path.as_deref(), since.as_deref())
        }
        ConfigSubcommands::Stats { config_type, path } => {
            show_config_stats(&mut config_manager, config_type, path.as_deref())
        }
        ConfigSubcommands::Schema { config_type, output } => {
            show_config_schema(config_type, output.as_deref())
        }
        ConfigSubcommands::Documentation { config_type, output } => {
...
fn show_config(
    ...
    let config_content = match config_type {
        ...
        _ => {
            return Err(crate::RhemaError::ConfigError(
                format!("Unknown config type: {}. Valid types: global, repository", config_type)
            ));
        }
    };
    ...
    match format {
        ...
        _ => {
            return Err(crate::RhemaError::ConfigError(
                format!("Unknown format: {}. Valid formats: json, yaml", format)
            ));
        }
    }
...
fn edit_config(
    ...
    let config_path: Result<PathBuf, crate::RhemaError> = match config_type {
        "global" => {
            ...
            Ok(PathBuf::from(home).join(".rhema").join("config").join("global.json"))
        },
        "repository" => {
            let repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
    ...
    let editor_cmd = editor.unwrap_or_else(|| {
...
fn validate_config(
    ...
    match config_type {
        "global" => {
            let result = config_manager.validation().validate_config(config_manager.global_config().clone(), "global")?;
            display_validation_report(&crate::config::validation::ValidationReport {
                overall_valid: result.valid,
                results: HashMap::new(),
                summary: crate::config::validation::ValidationSummary {
                    total_configs: 1,
                    valid_configs: if result.valid { 1 } else { 0 },
                    invalid_configs: if result.valid { 0 } else { 1 },
                    total_issues: result.issues.len(),
                    critical_issues: result.issues.iter().filter(|i| i.severity == crate::config::ConfigIssueSeverity::Critical).count(),
                    error_issues: result.issues.iter().filter(|i| i.severity == crate::config::ConfigIssueSeverity::Error).count(),
                    warning_issues: result.issues.iter().filter(|i| i.severity == crate::config::ConfigIssueSeverity::Warning).count(),
                    info_issues: result.issues.iter().filter(|i| i.severity == crate::config::ConfigIssueSeverity::Info).count(),
                },
        ...
        "repository" => {
            ...
            let result = config_manager.validation().validate_config(config_clone, repo_path)?;
            display_validation_report(&crate::config::validation::ValidationReport {
                overall_valid: result.valid,
                results: HashMap::new(),
                summary: crate::config::validation::ValidationSummary {
        ...
        _ => {
            return Err(crate::RhemaError::ConfigError(
                format!("Unknown config type: {}. Valid types: global, repository, all", config_type)
            ));
        }
    };
...
fn backup_config(
    ...
    let backup_report = match config_type {
        "global" => {
            ...
            let record = config_manager.backup_mut().backup_config(&global_config, "global")?;
            crate::config::backup::BackupReport {
                backups_created: vec![record],
                backups_failed: vec![],
                summary: crate::config::backup::BackupSummary {
                    total_backups: 1,
        ...
        "repository" => {
            ...
            let config = config_manager.load_repository_config(Path::new(repo_path))?;
            let config_clone = config.clone();
            let record = config_manager.backup_mut().backup_config(&config_clone, repo_path)?;
            crate::config::backup::BackupReport {
                backups_created: vec![record],
                backups_failed: vec![],
                summary: crate::config::backup::BackupSummary {
...
fn restore_config(
    ...
    let restore_report = match config_type {
        "global" => {
            let _config: GlobalConfig = config_manager.backup().restore_config("global", backup_file)?;
            // TODO: Implement proper restore report
            crate::config::backup::RestoreReport {
                restored_configs: vec![],
                restore_errors: vec![],
                summary: crate::config::backup::RestoreSummary {
                    total_configs: 1,
        ...
        "repository" => {
            ...
            let _config: RepositoryConfig = config_manager.backup().restore_config("repository", backup_file)?;
            // TODO: Implement proper restore report
            crate::config::backup::RestoreReport {
                restored_configs: vec![],
                restore_errors: vec![],
...
fn migrate_config(
    ...
    let migration_report = match config_type {
        "global" => {
            config_manager.migration().migrate_config(config_manager.global_config(), "global")?
        }
        "repository" => {
            let _repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
            ...
            let config_clone = config.clone();
            config_manager.migration().migrate_config(&config_clone, _repo_path)?
        }
        "all" => {
...
fn export_config(
    ...
    let export_content = match config_type {
        "global" => {
            config_manager.global_config().export(format)?
        }
        "repository" => {
            let _repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn import_config(
    ...
    match config_type {
        ...
        "repository" => {
            ...
            let mut repo_config = config_manager.load_repository_config(Path::new(_repo_path))?.clone();
    ...
    println!("✅ Configuration imported successfully from: {}", input.green());
...
fn set_config_value(
    ...
    println!("🔧 Setting configuration value: {} = {}", key.bright_blue(), value);
    
    let json_value = serde_json::from_str(value)?;
    
    match config_type {
        "global" => {
            config_manager.global_config_mut().set_value(key, json_value)?;
...
fn get_config_value(
    ...
    let value = match config_type {
        "global" => {
            config_manager.global_config().get_value(key)
        }
        "repository" => {
            let _repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn check_config_health(
    ...
    println!("🏥 Checking configuration health: {}", config_type.bright_blue());
    
    let health = match config_type {
        "global" => {
            &config_manager.global_config().health
        }
        "repository" => {
            let repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn audit_config(
    ...
    let audit_log = match config_type {
        "global" => {
            &config_manager.global_config().audit_log
        }
        "repository" => {
            let repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn show_config_stats(
    ...
    let stats = match config_type {
        "global" => {
            &config_manager.global_config().stats
        }
        "repository" => {
            let _repo_path = path.ok_or_else(|| {
                crate::RhemaError::ConfigError("Repository path required for repository config".to_string())
...
fn show_config_documentation(config_type: &str, output: Option<&str>) -> RhemaResult<()> {
    println!("📚 Configuration documentation: {}", config_type.bright_blue());
...
fn display_validation_report(
    ...
    println!("  Status: {}", if report.overall_valid { "✅ Valid".green() } else { "❌ Invalid".red() });
...
fn display_backup_report(
    report: &crate::config::backup::BackupReport,
    output: Option<&str>
) -> RhemaResult<()> {
    println!("Backup Report:");
    println!("  Status: {}", if report.backups_failed.is_empty() { "✅ Success".green() } else { "❌ Failed".red() });
    println!("  Files backed up: {}", report.backups_created.len());
    println!("  Backup location: {}", report.backups_created.first().map(|b| b.backup_path.display()).unwrap_or_else(|| std::path::Path::new("N/A").display()));
...
fn display_restore_report(
    report: &crate::config::backup::RestoreReport
) -> RhemaResult<()> {
    println!("Restore Report:");
    println!("  Status: {}", if report.restore_errors.is_empty() { "✅ Success".green() } else { "❌ Failed".red() });
...
fn display_migration_report(
    report: &crate::config::migration::MigrationReport
) -> RhemaResult<()> {
    println!("Migration Report:");
    println!("  Status: {}", if report.migrations_failed.is_empty() { "✅ Success".green() } else { "❌ Failed".red() });
    ...
    for migration in &report.migrations_applied {
        println!("    • {}: {}", migration.migration_name, if migration.success { "Success" } else { "Failed" });
    }
    
    Ok(())
}

fn display_config_health(
    health: &crate::config::ConfigHealth
) -> RhemaResult<()> {
    println!("Configuration Health:");
    println!("  Status: {}", match health.status {
        crate::config::ConfigHealthStatus::Healthy => "✅ Healthy".green(),
        crate::config::ConfigHealthStatus::Warning => "⚠️  Warning".yellow(),
        crate::config::ConfigHealthStatus::Error => "❌ Error".red(),
        crate::config::ConfigHealthStatus::Unknown => "❓ Unknown".blue(),
    });
...
fn display_audit_log(
    audit_log: &crate::config::ConfigAuditLog,
    since: Option<&str>
) -> RhemaResult<()> {
    println!("Configuration Audit Log:");
    println!("  Total changes: {}", audit_log.changes.len());
    ...
    for change in changes.iter().take(10) { // Show last 10 changes
        println!("    • {}: {} - {}", change.timestamp, change.change_type, change.description);
        println!("      User: {}, Path: {}", change.user, change.path.display());
    }
...
fn display_config_stats(
    stats: &crate::config::ConfigStats
) -> RhemaResult<()> {
