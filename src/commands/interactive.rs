...
impl InteractiveSession {
    ...
    fn handle_daemon(&mut self, args: &[&str]) -> RhemaResult<()> {
        ...
        let command = if args.contains(&"start") {
            crate::commands::daemon::DaemonSubcommand::Start {
                config: args.iter()
                    .position(|&s| s == "--config")
                    .and_then(|i| args.get(i + 1))
            ...
            crate::commands::daemon::DaemonSubcommand::Stop {
                pid_file: std::path::PathBuf::from("/tmp/rhema-mcp.pid"),
            }
        } else if args.contains(&"restart") {
            crate::commands::daemon::DaemonSubcommand::Restart {
                config: args.iter()
                    .position(|&s| s == "--config")
                    .and_then(|i| args.get(i + 1))
                    .map(|s| std::path::PathBuf::from(s)),
            ...
            crate::commands::daemon::DaemonSubcommand::Status {
                pid_file: std::path::PathBuf::from("/tmp/rhema-mcp.pid"),
            }
        } else {
            return Err(crate::error::RhemaError::InvalidCommand("daemon requires a subcommand: start, stop, restart, status".to_string()));
        };
        
        let args = crate::commands::daemon::DaemonArgs { command };
        
        tokio::runtime::Runtime::new()?.block_on(crate::commands::daemon::execute_daemon(args))
    }
...
pub fn run_interactive(rhema: Rhema) -> RhemaResult<()> {
    ...
    crate::commands::interactive_advanced::run_advanced_interactive(rhema)
}
...
pub fn run_interactive_with_config(
    ...
    crate::commands::interactive_advanced::run_advanced_interactive_with_config(
        rhema,
        config_file,
        no_auto_complete,
