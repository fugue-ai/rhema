...
async fn execute_integration_action(name: String, action: String, params: Option<String>) -> RhemaResult<()> {
    ...
    match metadata.integration_type {
        IntegrationType::Slack => {
            if let Some(slack_integration) = integration.as_ref().as_any().downcast_ref::<SlackIntegration>() {
                match action.as_str() {
                    "send_message" => {
        ...
        IntegrationType::Jira => {
            if let Some(jira_integration) = integration.as_ref().as_any().downcast_ref::<JiraIntegration>() {
                match action.as_str() {
                    "create_issue" => {
