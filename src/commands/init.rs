...
use crate::{Rhema, RhemaResult, RhemaScope, repo_analysis::RepoAnalysis};
use colored::*;
use std::fs;
use std::path::PathBuf;

pub fn run(rhema: &Rhema, scope_type: Option<&str>, scope_name: Option<&str>, auto_config: bool) -> RhemaResult<()> {
    ...
    if scope_path.exists() {
        return Err(crate::RhemaError::ConfigError(
            format!("Rhema scope already exists at {}", scope_path.display())
        ));
    }
    ...
    let (scope_type, scope_name, description, custom_fields) = if auto_config {
        ...
        let analysis = RepoAnalysis::analyze(&current_dir)?;
...
fn display_analysis_results(analysis: &RepoAnalysis) -> RhemaResult<()> {
    println!("\n📊 Repository Analysis Results:");
    println!("{}", "─".repeat(50));
    
    // Project type
    let project_type_str = match analysis.project_type {
        crate::repo_analysis::ProjectType::Monorepo => "Monorepo",
        crate::repo_analysis::ProjectType::Microservice => "Microservice",
        crate::repo_analysis::ProjectType::Monolithic => "Monolithic",
        crate::repo_analysis::ProjectType::Library => "Library",
        crate::repo_analysis::ProjectType::Application => "Application",
        crate::repo_analysis::ProjectType::Service => "Service",
        crate::repo_analysis::ProjectType::Unknown => "Unknown",
    };
    ...
    if !analysis.infrastructure.is_empty() {
        println!("🏗️  Infrastructure: {}", analysis.infrastructure.join(", ").yellow());
    }
...
fn create_default_protocol_info(scope_type: &str) -> crate::schema::ProtocolInfo {
    let concepts = vec![
        crate::schema::ConceptDefinition {
    ...
    let patterns = vec![
        crate::schema::PatternDefinition {
    ...
    let troubleshooting = vec![
        crate::schema::TroubleshootingItem {
