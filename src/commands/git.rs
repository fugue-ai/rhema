...
pub fn run(rhema: &Rhema, subcommand: &GitSubcommands) -> RhemaResult<()> {
    match subcommand {
        GitSubcommands::Hooks { subcommand } => {
            run_hooks(rhema, subcommand)
        }
        GitSubcommands::Workflow { subcommand } => {
            run_workflow(rhema, subcommand)
        }
        GitSubcommands::History { subcommand } => {
            run_history(rhema, subcommand)
        }
        GitSubcommands::Automation { subcommand } => {
            run_automation(rhema, subcommand)
        }
        GitSubcommands::Security { subcommand } => {
            run_security(rhema, subcommand)
        }
        GitSubcommands::Monitoring { subcommand } => {
            run_monitoring(rhema, subcommand)
        }
        GitSubcommands::Init { config, no_hooks, no_workflow, no_automation, no_security, no_monitoring } => {
            run_init(rhema, config.as_ref(), *no_hooks, *no_workflow, *no_automation, *no_security, *no_monitoring)
        }
        GitSubcommands::Status => {
            run_status(rhema)
        }
        GitSubcommands::Shutdown => {
            run_shutdown(rhema)
        }
        GitSubcommands::Advanced { subcommand } => {
            run_advanced(rhema, subcommand)
        }
    }
}
...
fn run_hooks(rhema: &Rhema, subcommand: &HookSubcommands) -> RhemaResult<()> {
    ...
    let git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    match subcommand {
                ...
                match git_integration.execute_hook(hook_type) {
                    Ok(result) => {
                        if result.success {
                            ...
                            println!("⚠ {} hook installed with warnings: {:?}", hook_type.filename(), result.warnings);
                        }
                    }
                    Err(e) => {
        ...
        HookSubcommands::Execute { hook } => {
            ...
            let hook_type = match hook.as_str() {
                ...
                _ => {
                    return Err(crate::RhemaError::InvalidInput(
                        format!("Unknown hook type: {}", hook)
                    ));
                }
            };
        ...
        HookSubcommands::Status => {
            ...
            for (hook_type, status) in &integration_status.hook_status {
                ...
                println!("    {} {}: {}", status_symbol, hook_type.filename(), if *status { "Active" } else { "Inactive" });
            }
...
fn run_workflow(rhema: &Rhema, subcommand: &WorkflowSubcommands) -> RhemaResult<()> {
    ...
    match subcommand {
        ...
        WorkflowSubcommands::AnalyzePr { pr_number } => {
            ...
            println!("  Affected scopes: {}", analysis.impact_analysis.affected_scopes.len());
            println!("  Breaking changes: {}", analysis.impact_analysis.breaking_changes.len());
...
fn run_history(rhema: &Rhema, subcommand: &HistorySubcommands) -> RhemaResult<()> {
    ...
    match subcommand {
        ...
        HistorySubcommands::Version { version, version_type, description } => {
            ...
            let context_version = git_integration.create_context_version(version, version_type_enum, description)?;
        ...
        AutomationSubcommands::Config => {
            ...
            println!("  Success rate: {:.1}%", 
                if automation_status.total_tasks > 0 {
                    (automation_status.completed_tasks as f64 / automation_status.total_tasks as f64) * 100.0
                } else {
                    0.0
                }
            );
...
/// Run initialization
fn run_init(rhema: &Rhema, config: Option<&PathBuf>, no_hooks: bool, no_workflow: bool, no_automation: bool, no_security: bool, no_monitoring: bool) -> RhemaResult<()> {
    ...
    let mut git_integration = if let Some(config_path) = config {
        ...
        git::create_advanced_git_integration_with_config(repo_path, config)?
    } else {
        git::create_advanced_git_integration(repo_path)?
    };
    ...
    if !no_workflow {
        ...
        let workflow_status = git_integration.get_workflow_status()?;
        println!("✓ Git workflow initialized: {:?}", workflow_status.workflow_type);
    }
    ...
    println!("  Hooks installed: {}", integration_status.hooks_installed);
    println!("  Workflow active: {}", integration_status.workflow_status.status);
    println!("  Automation running: {}", integration_status.automation_status.running);
...
fn run_status(rhema: &Rhema) -> RhemaResult<()> {
    ...
    println!("Integration: {}", if integration_status.enabled { "✓ Enabled" } else { "✗ Disabled" });
    println!("Hooks: {}", if integration_status.hooks_installed { "✓ Installed" } else { "✗ Not installed" });
    println!("Workflow: {}", integration_status.workflow_status.status);
    println!("Automation: {}", if integration_status.automation_status.running { "✓ Running" } else { "✗ Stopped" });
    ...
    println!("\nWorkflow Details:");
    println!("  Type: {:?}", integration_status.workflow_status.workflow_type);
    println!("  Current branch: {}", integration_status.workflow_status.current_branch);
    println!("  Branch type: {:?}", integration_status.workflow_status.branch_type);
    
    println!("\nAutomation Details:");
    println!("  Total tasks: {}", integration_status.automation_status.total_tasks);
    println!("  Completed: {}", integration_status.automation_status.completed_tasks);
    println!("  Failed: {}", integration_status.automation_status.failed_tasks);
    println!("  Pending: {}", integration_status.automation_status.pending_tasks);
        ...
        SecuritySubcommands::Scan { path } => {
            ...
            if !scan_result.issues.is_empty() {
                ...
                for (i, issue) in scan_result.issues.iter().enumerate() {
                    println!("    {}. {} ({}): {}", i + 1, issue.severity, issue.category, issue.description);
                }
            }
            
            Ok(())
        }
        SecuritySubcommands::ValidateAccess { user, operation, resource } => {
            println!("Validating access for user {} to perform {} on {}", user, operation, resource);
...
fn run_monitoring(rhema: &Rhema, subcommand: &MonitoringSubcommands) -> RhemaResult<()> {
    ...
    match subcommand {
        ...
        MonitoringSubcommands::Performance => {
            ...
            git_integration.record_context_operation("validate", chrono::Duration::milliseconds(75))?;
        ...
        MonitoringSubcommands::Events { limit } => {
            let limit_str = limit.map(|l| l.to_string()).unwrap_or_else(|| "all".to_string());
            println!("Recent events (limit: {}):", limit_str);
            
            // Record some sample events for demonstration
            git_integration.record_git_operation("feature_branch_created", chrono::Duration::milliseconds(100))?;
            git_integration.record_context_operation("validation_passed", chrono::Duration::milliseconds(50))?;
            git_integration.record_git_operation("release_merged", chrono::Duration::milliseconds(200))?;
...
fn run_advanced(rhema: &Rhema, subcommand: &AdvancedSubcommands) -> RhemaResult<()> {
    match subcommand {
        AdvancedSubcommands::Hooks { install, configure, test, status } => {
            run_advanced_hooks(rhema, *install, *configure, *test, *status)
        }
        AdvancedSubcommands::BranchContext { init, validate, merge, backup, restore, branch } => {
            run_advanced_branch_context(rhema, *init, *validate, *merge, *backup, *restore, branch.as_deref())
        }
        AdvancedSubcommands::Workflow { init, configure, start_feature, finish_feature, start_release, finish_release, analyze_pr, feature, version, pr_number } => {
            run_advanced_workflow(rhema, *init, *configure, *start_feature, *finish_feature, *start_release, *finish_release, *analyze_pr, feature.as_deref(), version.as_deref(), *pr_number)
        }
        AdvancedSubcommands::History { track, report, blame, version, rollback, scope, file, version_id } => {
            run_advanced_history(rhema, *track, *report, *blame, *version, *rollback, scope.as_deref(), file.as_ref(), version_id.as_deref())
        }
        AdvancedSubcommands::Automation { start, stop, configure, status, history, cancel, task_id } => {
            run_advanced_automation(rhema, *start, *stop, *configure, *status, *history, *cancel, task_id.as_deref())
        }
        AdvancedSubcommands::Security { scan, validate_access, validate_commit, encrypt, decrypt, user, operation, resource, commit, file } => {
            run_advanced_security(rhema, *scan, *validate_access, *validate_commit, *encrypt, *decrypt, user.as_deref(), operation.as_deref(), resource.as_deref(), commit.as_deref(), file.as_ref())
        }
        AdvancedSubcommands::Monitoring { start, stop, status, performance, analytics, events, metric, limit } => {
            run_advanced_monitoring(rhema, *start, *stop, *status, *performance, *analytics, *events, metric.as_deref(), *limit)
        }
        AdvancedSubcommands::Integration { init, configure, test, status, backup, restore } => {
            run_advanced_integration(rhema, *init, *configure, *test, *status, *backup, *restore)
        }
    }
}

fn run_advanced_hooks(rhema: &Rhema, install: bool, configure: bool, test: bool, status: bool) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let git_integration = git::create_advanced_git_integration(repo_path)?;
...
fn run_advanced_branch_context(rhema: &Rhema, init: bool, validate: bool, merge: bool, backup: bool, restore: bool, branch: Option<&str>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let mut git_integration = git::create_advanced_git_integration(repo_path)?;
...
fn run_advanced_workflow(rhema: &Rhema, init: bool, configure: bool, start_feature: bool, finish_feature: bool, start_release: bool, finish_release: bool, analyze_pr: bool, feature: Option<&str>, version: Option<&str>, pr_number: Option<u64>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let mut git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    if start_feature {
        let feature_name = feature.ok_or_else(|| RhemaError::InvalidInput("Feature name required".to_string()))?;
    ...
    if start_release {
        let release_version = version.ok_or_else(|| RhemaError::InvalidInput("Release version required".to_string()))?;
    ...
    if analyze_pr {
        let pr_num = pr_number.ok_or_else(|| RhemaError::InvalidInput("Pull request number required".to_string()))?;
...
fn run_advanced_history(rhema: &Rhema, track: bool, report: bool, blame: bool, version: bool, rollback: bool, scope: Option<&str>, file: Option<&PathBuf>, version_id: Option<&str>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let mut git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    if blame {
        let file_path = file.ok_or_else(|| RhemaError::InvalidInput("File path required".to_string()))?;
    ...
    if version {
        let version_str = version_id.ok_or_else(|| RhemaError::InvalidInput("Version identifier required".to_string()))?;
        println!("Creating context version: {}", version_str);
        let context_version = git_integration.create_context_version(version_str, git::history::VersionType::Patch, "Advanced version creation")?;
        println!("Context version created: {}", context_version.version);
    }
...
fn run_advanced_automation(rhema: &Rhema, start: bool, stop: bool, configure: bool, status: bool, history: bool, cancel: bool, task_id: Option<&str>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    if cancel {
        let task = task_id.ok_or_else(|| RhemaError::InvalidInput("Task ID required".to_string()))?;
...
fn run_advanced_security(rhema: &Rhema, scan: bool, validate_access: bool, validate_commit: bool, encrypt: bool, decrypt: bool, user: Option<&str>, operation: Option<&str>, resource: Option<&str>, commit: Option<&str>, file: Option<&PathBuf>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    if scan {
        ...
        let scan_result = git_integration.run_security_scan(scan_path)?;
        println!("Security scan completed: {} issues found", scan_result.issues.len());
    }
    
    if validate_access {
        let user_name = user.ok_or_else(|| RhemaError::InvalidInput("User name required".to_string()))?;
        let op = operation.ok_or_else(|| RhemaError::InvalidInput("Operation required".to_string()))?;
        let res = resource.ok_or_else(|| RhemaError::InvalidInput("Resource required".to_string()))?;
        println!("Validating access for user: {} operation: {} resource: {}", user_name, op, res);
        let has_access = git_integration.validate_access(user_name, &git::security::Operation::Read, res)?;
        println!("Access validation result: {}", has_access);
    }
    
    if validate_commit {
        let commit_hash = commit.ok_or_else(|| RhemaError::InvalidInput("Commit hash required".to_string()))?;
        println!("Validating commit security: {}", commit_hash);
        let validation_result = git_integration.validate_commit_security(commit_hash)?;
        println!("Commit security validation completed: {}", validation_result.is_valid);
    }
...
fn run_advanced_monitoring(rhema: &Rhema, start: bool, stop: bool, status: bool, performance: bool, analytics: bool, events: bool, _metric: Option<&str>, limit: Option<usize>) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let git_integration = git::create_advanced_git_integration(repo_path)?;
...
fn run_advanced_integration(rhema: &Rhema, init: bool, configure: bool, test: bool, status: bool, backup: bool, restore: bool) -> RhemaResult<()> {
    let repo_path = rhema.repo_path();
    let mut git_integration = git::create_advanced_git_integration(repo_path)?;
    ...
    if status {
        ...
        println!("  Integration enabled: {}", integration_status.enabled);
        println!("  Hooks installed: {}", integration_status.hooks_installed);
        println!("  Workflow status: {:?}", integration_status.workflow_status);
        println!("  Automation status: {:?}", integration_status.automation_status);
    }
