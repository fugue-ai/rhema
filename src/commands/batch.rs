...
pub fn run(rhema: &Rhema, subcommand: &BatchSubcommands) -> RhemaResult<()> {
    match subcommand {
        BatchSubcommands::Context { operation, input_file, scope_filter, dry_run } => {
            run_context_operations(rhema, operation, input_file, scope_filter.as_deref(), *dry_run)
        }
        BatchSubcommands::Commands { command_file, scope_filter, parallel, max_workers } => {
            run_command_execution(rhema, command_file, scope_filter.as_deref(), *parallel, *max_workers)
        }
        BatchSubcommands::Data { operation, input_path, output_path, format, scope_filter } => {
            run_data_operations(rhema, operation, input_path, output_path, format, scope_filter.as_deref())
        }
        BatchSubcommands::Validate { validation_type, scope_filter, output_file, detailed } => {
            run_validation_operations(rhema, validation_type, scope_filter.as_deref(), output_file.as_deref(), *detailed)
        }
        BatchSubcommands::Report { report_type, scope_filter, output_file, format, include_details } => {
            run_reporting_operations(rhema, report_type, scope_filter.as_deref(), output_file, format, *include_details)
        }
    }
}

/// Batch context file operations
pub fn run_context_operations(rhema: &Rhema, operation: &str, input_file: &str, scope_filter: Option<&str>, dry_run: bool) -> RhemaResult<()> {
    ...
    let filtered_scopes = if let Some(filter) = scope_filter {
        scopes.into_iter().filter(|scope| scope.path.to_string_lossy().contains(filter)).collect()
    } else {
        scopes
    };
        ...
        match operation {
            ...
            _ => {
                return Err(crate::RhemaError::ConfigError(
                    format!("Unknown batch operation: {}", operation)
                ));
            }
        }
...
pub fn run_command_execution(rhema: &Rhema, command_file: &str, scope_filter: Option<&str>, parallel: bool, _max_workers: usize) -> RhemaResult<()> {
    ...
    if parallel {
        ...
        let results: Vec<BatchItemResult> = filtered_scopes.par_iter()
            .flat_map(|scope| {
        ...
        for scope in &filtered_scopes {
            for cmd in &commands {
                progress_bar.set_message(format!("Executing '{}' on scope: {}", cmd.command, scope.path.display()));
...
pub fn run_data_operations(rhema: &Rhema, operation: &str, input_path: &str, output_path: &str, format: &str, scope_filter: Option<&str>) -> RhemaResult<()> {
    ...
    match operation {
        "export" => {
            ...
            for scope in &filtered_scopes {
                progress_bar.set_message(format!("Exporting data from scope: {}", scope.path.display()));
        ...
        "import" => {
            ...
            for scope in &filtered_scopes {
                progress_bar.set_message(format!("Importing data to scope: {}", scope.path.display()));
        ...
        _ => {
            return Err(crate::RhemaError::ConfigError(
                format!("Unknown data operation: {}", operation)
            ));
        }
    }
...
pub fn run_validation_operations(rhema: &Rhema, operation: &str, scope_filter: Option<&str>, output_file: Option<&str>, detailed: bool) -> RhemaResult<()> {
    ...
    for scope in &filtered_scopes {
        ...
        let item_result = match operation {
            ...
            _ => {
                return Err(crate::RhemaError::ConfigError(
                    format!("Unknown validation operation: {}", operation)
                ));
            }
        };
...
/// Batch reporting and analytics
pub fn run_reporting_operations(rhema: &Rhema, report_type: &str, scope_filter: Option<&str>, output_file: &str, format: &str, include_details: bool) -> RhemaResult<()> {
    ...
    for scope in &filtered_scopes {
        progress_bar.set_message(format!("Collecting data from scope: {}", scope.path.display()));
...
fn read_batch_input_file(input_file: &str) -> RhemaResult<HashMap<String, serde_yaml::Value>> {
    ...
    if !path.exists() {
        return Err(crate::RhemaError::FileNotFound(
            format!("Batch input file not found: {}", input_file)
        ));
    }
...
fn read_batch_commands_file(command_file: &str) -> RhemaResult<Vec<BatchCommand>> {
    ...
    if !path.exists() {
        return Err(crate::RhemaError::FileNotFound(
            format!("Batch command file not found: {}", command_file)
        ));
    }
    ...
    let commands: Vec<BatchCommand> = serde_yaml::from_str(&content)
        .map_err(|e| crate::RhemaError::InvalidYaml {
...
fn batch_validate_scope(_rhema: &Rhema, scope: &crate::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    ...
    // Validate YAML files
    let yaml_files = ["knowledge.yaml", "todos.yaml", "decisions.yaml", "patterns.yaml", "conventions.yaml"];
...
fn batch_migrate_scope(rhema: &Rhema, scope: &crate::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    let target_version = input_data.get("target_version")
        .and_then(|v| v.as_str())
        .unwrap_or("1.0.0");
    
    let backup_files = input_data.get("backup_files")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let backup_directory = input_data.get("backup_directory")
        .and_then(|v| v.as_str())
        .unwrap_or("./backups");
    ...
    // Perform migration
    match crate::commands::migrate::run(rhema, false, false) {
        Ok(_) => {
...
fn batch_migrate_scope_dry_run(_rhema: &Rhema, scope: &crate::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
...
fn batch_export_scope(rhema: &Rhema, scope: &crate::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    let format = input_data.get("format")
        .and_then(|v| v.as_str())
        .unwrap_or("json");
    
    let output_directory = input_data.get("output_directory")
        .and_then(|v| v.as_str())
        .unwrap_or("./exports");
    
    let include_protocol = input_data.get("include_protocol")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let include_knowledge = input_data.get("include_knowledge")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let include_todos = input_data.get("include_todos")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let include_decisions = input_data.get("include_decisions")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let include_patterns = input_data.get("include_patterns")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let include_conventions = input_data.get("include_conventions")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let summarize = input_data.get("summarize")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let ai_agent_format = input_data.get("ai_agent_format")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    
    let _compress_output = input_data.get("compress_output")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    let scope_name = scope_path.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("unknown");
    let output_file = output_path.join(format!("{}_{}.{}", scope_name, timestamp, format));
    
    // Perform export
    match crate::commands::export_context::run(
        rhema,
        format,
        ...
        Ok(_) => {
            ...
            metadata.insert("output_file".to_string(), output_file.to_string_lossy().to_string());
...
fn batch_import_scope(_rhema: &Rhema, scope: &crate::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
...
fn batch_import_scope_dry_run(_rhema: &Rhema, scope: &crate::scope::Scope, _input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
...
fn batch_health_check_scope(rhema: &Rhema, scope: &crate::scope::Scope, input_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    let check_dependencies = input_data.get("check_dependencies")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let check_file_permissions = input_data.get("check_file_permissions")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let check_schema_compliance = input_data.get("check_schema_compliance")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let check_data_integrity = input_data.get("check_data_integrity")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let _generate_report = input_data.get("generate_report")
        .and_then(|v| v.as_bool())
        .unwrap_or(true);
    
    let _report_format = input_data.get("report_format")
        .and_then(|v| v.as_str())
        .unwrap_or("json");
    
    // Perform health check
    match crate::commands::health::run(rhema, Some(scope.path.to_str().unwrap_or(""))) {
        Ok(_) => {
            ...
            if check_dependencies {
                ...
                metadata.insert("dependencies_healthy".to_string(), dep_result.success.to_string());
            ...
            if check_schema_compliance {
                ...
                metadata.insert("schema_compliant".to_string(), schema_result.success.to_string());
...
fn execute_command_on_scope(rhema: &Rhema, scope: &crate::scope::Scope, cmd: &BatchCommand) -> BatchItemResult {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    let result = match cmd.command.as_str() {
        "validate" => {
            ...
            match crate::commands::validate::run(rhema, recursive, false, false) {
                Ok(_) => {
        ...
        "stats" => {
            ...
            match crate::commands::stats::run(rhema) {
                Ok(_) => {
        ...
        "query" => {
            ...
            if query.is_empty() {
                ...
                match crate::commands::query::run_formatted(rhema, query, "yaml") {
                    Ok(_) => {
        ...
        "search" => {
            ...
            let term = args.get("term")
                .and_then(|v| v.as_str())
                .unwrap_or("");
            let in_file = args.get("in_file")
                .and_then(|v| v.as_str());
            let regex = args.get("regex")
                .and_then(|v| v.as_bool())
                .unwrap_or(false);
            ...
            if term.is_empty() {
                ...
                match crate::commands::search::run(rhema, term, in_file, regex) {
                    Ok(_) => {
...
fn export_scope_data(_rhema: &Rhema, scope: &crate::scope::Scope, all_data: &mut HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    all_data.insert(scope.path.to_string_lossy().to_string(), serde_yaml::Value::Mapping(
        scope_data.into_iter().map(|(k, v)| (serde_yaml::Value::String(k), v)).collect()
    ));
...
fn read_import_data(input_path: &str, format: &str) -> RhemaResult<HashMap<String, serde_yaml::Value>> {
    let path = Path::new(input_path);
    if !path.exists() {
        return Err(crate::RhemaError::FileNotFound(
            format!("Import data file not found: {}", input_path)
        ));
    }
    ...
    match format {
        ...
        _ => {
            Err(crate::RhemaError::ConfigError(
...
fn import_scope_data(_rhema: &Rhema, scope: &crate::scope::Scope, import_data: &HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    ...
    if let serde_yaml::Value::Mapping(files) = scope_data {
        for (file_name, file_data) in files {
            if let serde_yaml::Value::String(name) = file_name {
                ...
                let content = serde_yaml::to_string(file_data)
                    .map_err(|e| crate::RhemaError::ConfigError(format!("Failed to serialize {}: {}", name, e)))?;
...
fn write_export_data(all_data: &HashMap<String, serde_yaml::Value>, output_path: &str, format: &str) -> RhemaResult<()> {
    ...
    let content = match format {
        "json" => {
            serde_json::to_string_pretty(all_data)
                .map_err(|e| crate::RhemaError::ConfigError(format!("Failed to serialize to JSON: {}", e)))?
        }
        "yaml" | "yml" => {
            serde_yaml::to_string(all_data)
                .map_err(|e| crate::RhemaError::ConfigError(format!("Failed to serialize to YAML: {}", e)))?
        }
        _ => {
            return Err(crate::RhemaError::ConfigError(
...
fn batch_validate_scope_comprehensive(_rhema: &Rhema, scope: &crate::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    match file_ops::read_yaml_file::<crate::schema::RhemaScope>(&rhema_file) {
        Ok(scope_def) => {
...
fn batch_health_check_scope_comprehensive(_rhema: &Rhema, scope: &crate::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    // Validate rhema.yaml content
    let scope_def = match file_ops::read_yaml_file::<crate::schema::RhemaScope>(&rhema_file) {
        Ok(def) => def,
        Err(e) => {
            return Ok(BatchItemResult {
                item_id: scope.path.to_string_lossy().to_string(),
    ...
    let required_files = ["knowledge.yaml", "todos.yaml", "decisions.yaml", "patterns.yaml"];
    ...
    if let Some(dependencies) = &scope_def.dependencies {
        ...
        metadata.insert("total_dependencies".to_string(), dependencies.len().to_string());
...
fn batch_schema_check_scope(_rhema: &Rhema, scope: &crate::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    if let Some(rhema_file) = find_scope_file(scope_path) {
        match file_ops::read_yaml_file::<crate::schema::RhemaScope>(&rhema_file) {
            Ok(scope_def) => {
                metadata.insert("scope_schema_version".to_string(), scope_def.version.clone());
                
                // Check for required fields
                if scope_def.name.is_empty() {
                    warnings.push("Scope name is empty".to_string());
                }
                if scope_def.description.as_ref().map_or(true, |d| d.is_empty()) {
        ...
        if file_path.exists() {
            match validate_schema_compliance(&file_path, schema_type) {
                Ok(validation_result) => {
                    if detailed {
                        metadata.insert(format!("{}_schema_valid", schema_type), "true".to_string());
                    }
                ...
                Err(e) => {
                    ...
                    if detailed {
                        metadata.insert(format!("{}_schema_valid", schema_type), "false".to_string());
...
fn batch_dependency_check_scope(rhema: &Rhema, scope: &crate::scope::Scope, detailed: bool) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let mut metadata = HashMap::new();
    ...
    if let Some(dependencies) = &scope_def.dependencies {
        for dep in dependencies {
            ...
            match file_ops::read_yaml_file::<crate::schema::RhemaScope>(&dep_rhema_file) {
                Ok(dep_def) => {
                    valid_deps += 1;
                    ...
                    if let Some(dep_deps) = &dep_def.dependencies {
                        for dep_dep in dep_deps {
                            if dep_dep.path == scope.path.to_string_lossy() {
                                circular_deps.push(format!("Circular dependency: {} -> {}", scope.path.display(), dep.path));
                            }
                        }
                    }
        ...
        metadata.insert("valid_dependencies".to_string(), valid_deps.to_string());
        metadata.insert("total_dependencies".to_string(), dependencies.len().to_string());
        metadata.insert("invalid_dependencies".to_string(), invalid_deps.len().to_string());
        metadata.insert("missing_dependencies".to_string(), missing_deps.len().to_string());
        metadata.insert("circular_dependencies".to_string(), circular_deps.len().to_string());
    }
    ...
    if !circular_deps.is_empty() {
        warnings.push(format!("Circular dependencies: {}", circular_deps.join(", ")));
    }
    ...
    if detailed {
        ...
        for other_scope in &all_scopes {
            ...
            if let Ok(other_def) = file_ops::read_yaml_file::<crate::schema::RhemaScope>(&other_rhema_file) {
                if let Some(other_deps) = &other_def.dependencies {
                    for other_dep in other_deps {
                        if other_dep.path == scope.path.to_string_lossy() {
                            ...
                            let is_listed = scope_def.dependencies.as_ref()
                                .map(|deps| deps.iter().any(|d| d.path == other_scope.path.to_string_lossy()))
                                .unwrap_or(false);
        ...
        if !orphaned_refs.is_empty() {
            warnings.push(format!("Orphaned references: {}", orphaned_refs.iter().map(|p| p.to_string_lossy()).collect::<Vec<_>>().join(", ")));
        }
        
        metadata.insert("orphaned_references".to_string(), orphaned_refs.len().to_string());
    }
...
fn write_validation_report(result: &BatchOperationResult, output_file: &str) -> RhemaResult<()> {
    ...
    for detail in &result.details {
        report.push_str(&format!("### {}\n", detail.item_id));
        report.push_str(&format!("**Status:** {}\n", if detail.success { "✅ Success" } else { "❌ Failed" }));
...
fn collect_scope_report_data(rhema: &Rhema, scope: &crate::scope::Scope, report_type: &str, report_data: &mut HashMap<String, serde_yaml::Value>) -> RhemaResult<BatchItemResult> {
    let mut warnings = Vec::new();
    let metadata = HashMap::new();
    ...
    // Read scope definition
    if let Some(rhema_file) = find_scope_file(scope_path) {
        if let Ok(scope_def) = file_ops::read_yaml_file::<crate::schema::RhemaScope>(&rhema_file) {
            scope_report.insert("name".to_string(), serde_yaml::Value::String(scope_def.name));
            scope_report.insert("version".to_string(), serde_yaml::Value::String(scope_def.version));
            scope_report.insert("description".to_string(), serde_yaml::Value::String(scope_def.description.unwrap_or_default()));
        }
    }
    ...
    match report_type {
        ...
        "analytics" => {
            ...
            if knowledge_file.exists() {
                if let Ok(knowledge) = file_ops::read_yaml_file::<crate::schema::Knowledge>(&knowledge_file) {
                    analytics.insert("knowledge_count".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(knowledge.entries.len())));
                }
            }
            ...
            if todos_file.exists() {
                if let Ok(todos) = file_ops::read_yaml_file::<crate::schema::Todos>(&todos_file) {
                    analytics.insert("todos_count".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(todos.todos.len())));
                    ...
                    analytics.insert("todo_status_counts".to_string(), serde_yaml::to_value(status_counts)?);
                }
            }
            ...
            if decisions_file.exists() {
                if let Ok(decisions) = file_ops::read_yaml_file::<crate::schema::Decisions>(&decisions_file) {
                    analytics.insert("decisions_count".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(decisions.decisions.len())));
                }
            }
            ...
            if patterns_file.exists() {
                if let Ok(patterns) = file_ops::read_yaml_file::<crate::schema::Patterns>(&patterns_file) {
                    analytics.insert("patterns_count".to_string(), serde_yaml::Value::Number(serde_yaml::Number::from(patterns.patterns.len())));
                }
            }
        ...
        "health" => {
            ...
            scope_report.insert("health_status".to_string(), serde_yaml::Value::Bool(health_result.success));
            scope_report.insert("health_warnings".to_string(), serde_yaml::to_value(health_result.warnings)?);
            scope_report.insert("health_metadata".to_string(), serde_yaml::to_value(health_result.metadata)?);
        }
        "dependencies" => {
            // Collect dependency data
            let dep_result = batch_dependency_check_scope(rhema, scope, true)?;
            scope_report.insert("dependency_status".to_string(), serde_yaml::Value::Bool(dep_result.success));
            scope_report.insert("dependency_warnings".to_string(), serde_yaml::to_value(dep_result.warnings)?);
            scope_report.insert("dependency_metadata".to_string(), serde_yaml::to_value(dep_result.metadata)?);
        }
        "todos" => {
            // Collect todos data
            let todos_file = scope_path.join("todos.yaml");
    ...
    report_data.insert(scope.path.to_string_lossy().to_string(), serde_yaml::Value::Mapping(
        scope_report.into_iter().map(|(k, v)| (serde_yaml::Value::String(k), v)).collect()
    ));
...
fn generate_batch_report(report_type: &str, report_data: &HashMap<String, serde_yaml::Value>, include_details: bool) -> RhemaResult<String> {
    let mut report = String::new();
    
    // Add header
    report.push_str(&format!("# Rhema Batch Report - {}\n\n", report_type.to_uppercase()));
    report.push_str(&format!("Generated: {}\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S UTC")));
    ...
    match report_type {
        "summary" => {
            ...
            for (scope_path, scope_data) in report_data {
                ...
                if let serde_yaml::Value::Mapping(data) = scope_data {
                    ...
                    if let Some(version) = data.get(&serde_yaml::Value::String("version".to_string())) {
                    ...
                    if let Some(files) = data.get(&serde_yaml::Value::String("files".to_string())) {
                        if let serde_yaml::Value::Mapping(files_map) = files {
                            ...
                            for (file_name, exists) in files_map {
                                if let (serde_yaml::Value::String(name), serde_yaml::Value::Bool(exists_bool)) = (file_name, exists) {
                                    if *exists_bool {
                                        file_count += 1;
                                        report.push_str(&format!("- ✅ {}\n", name));
                                    } else {
        ...
        "analytics" => {
            ...
            for (scope_path, scope_data) in report_data {
                ...
                if let serde_yaml::Value::Mapping(data) = scope_data {
                    if let Some(analytics) = data.get(&serde_yaml::Value::String("analytics".to_string())) {
                                        ...
                                        "decisions_count" => {
                                            if let serde_yaml::Value::Number(n) = value {
                                                if let Some(count) = n.as_u64() {
                                                    ...
                                                    report.push_str(&format!("- Decisions: {}\n", count));
                                                }
                                            }
                                        }
                                        "patterns_count" => {
        ...
        "health" => {
            ...
            for (scope_path, scope_data) in report_data {
                ...
                if let serde_yaml::Value::Mapping(data) = scope_data {
                    if let Some(health_status) = data.get(&serde_yaml::Value::String("health_status".to_string())) {
                    ...
                    if let Some(warnings) = data.get(&serde_yaml::Value::String("health_warnings".to_string())) {
            ...
            report.push_str(&format!("- Health rate: {:.1}%\n", 
                if report_data.is_empty() { 0.0 } else { (healthy_scopes as f64 / report_data.len() as f64) * 100.0 }));
        }
        _ => {
...
fn convert_markdown_to_html(markdown: &str) -> String {
    ...
    html.push_str("pre { background-color: #f4f4f4; padding: 10px; border-radius: 5px; overflow-x: auto; }\n");
    html.push_str("</style>\n");
    html.push_str("</head>\n<body>\n");
...
fn copy_dir_recursive(source: &Path, destination: &Path) -> RhemaResult<()> {
    if !source.is_dir() {
        return Err(crate::RhemaError::ConfigError("Source is not a directory".to_string()));
    }
        ...
        .filter_map(|e| e.ok())
    {
        ...
        let relative_path = source_file.strip_prefix(source)
            .map_err(|e| crate::RhemaError::ConfigError(format!("Failed to get relative path: {}", e)))?;
...
fn validate_yaml_file(file_path: &Path, schema_type: &str) -> RhemaResult<()> {
    match schema_type {
        "knowledge" => {
            let _: crate::schema::Knowledge = file_ops::read_yaml_file(file_path)?;
        }
        "todos" => {
            let _: crate::schema::Todos = file_ops::read_yaml_file(file_path)?;
        }
        "decisions" => {
            let _: crate::schema::Decisions = file_ops::read_yaml_file(file_path)?;
        }
        "patterns" => {
            let _: crate::schema::Patterns = file_ops::read_yaml_file(file_path)?;
        }
        "conventions" => {
            let _: crate::schema::Conventions = file_ops::read_yaml_file(file_path)?;
        }
        _ => {
...
fn validate_schema_compliance(file_path: &Path, schema_type: &str) -> RhemaResult<SchemaValidationResult> {
    let mut warnings = Vec::new();
    
    match schema_type {
        "knowledge" => {
            let knowledge: crate::schema::Knowledge = file_ops::read_yaml_file(file_path)?;
            ...
            for (i, entry) in knowledge.entries.iter().enumerate() {
                ...
                if let Some(confidence) = entry.confidence {
                    if confidence > 100 {
                        warnings.push(format!("Knowledge entry {} has invalid confidence value: {}", i, confidence));
                    }
                }
            }
        }
        "todos" => {
            let todos: crate::schema::Todos = file_ops::read_yaml_file(file_path)?;
        ...
        "decisions" => {
            let decisions: crate::schema::Decisions = file_ops::read_yaml_file(file_path)?;
        ...
        "patterns" => {
            let patterns: crate::schema::Patterns = file_ops::read_yaml_file(file_path)?;
...
fn print_batch_results(result: &BatchOperationResult) {
    ...
    if result.failed > 0 {
        ...
        for detail in &result.details {
            if !detail.success {
                println!("  - {}: {}", detail.item_id, detail.error.as_ref().unwrap_or(&"Unknown error".to_string()));
            }
        }
    }
