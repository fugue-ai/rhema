...
use crate::{Rhema, RhemaResult, RhemaScope, schema::{ProtocolInfo, ConceptDefinition, CqlExample, PatternDefinition, IntegrationGuide, TroubleshootingItem}};
use colored::*;
use serde_json;
    ...
    for scope in scopes {
        if let Ok(conventions) = rhema.load_conventions(&scope.name) {
            for convention in &conventions.conventions {
                *by_type.entry(convention.convention_type.clone()).or_insert(0) += 1;
                *by_enforcement.entry(format!("{:?}", convention.enforcement)).or_insert(0) += 1;
        ...
        "json" => {
            if ai_agent_format {
                ...
                serde_json::to_string_pretty(data)
                    .map_err(|e| crate::RhemaError::JsonError(e))
            }
        }
        "yaml" => {
            if ai_agent_format {
                ...
                serde_yaml::to_string(data)
                    .map_err(|e| crate::RhemaError::YamlError(e))
            }
        }
...
fn format_ai_agent_json(data: &ContextExport) -> RhemaResult<String> {
    ...
    serde_json::to_string_pretty(&ai_format)
        .map_err(|e| crate::RhemaError::JsonError(e))
}
...
fn format_ai_agent_yaml(data: &ContextExport) -> RhemaResult<String> {
    ...
    serde_yaml::to_string(&ai_format)
        .map_err(|e| crate::RhemaError::YamlError(e))
}
...
fn format_markdown(data: &ContextExport, ai_agent_format: bool) -> RhemaResult<String> {
    ...
    if ai_agent_format {
        ...
        if let Some(ref conventions) = data.conventions {
            ...
            md.push_str(&format!("**Total Conventions:** {}\n", conventions.convention_count));
            md.push_str("**By Type:**\n");
