...
use super::{Config, ConfigError, ConfigChange, ConfigChangeType};
...
impl MigrationManager {
    ...
    pub fn migrate_config<T: Config>(&self, config: &T, context: &str) -> RhemaResult<MigrationReport> {
        ...
        let target_version = Version::parse(super::CURRENT_CONFIG_VERSION)
            .map_err(|e| ConfigError::VersionMismatch {
                expected: "valid semver".to_string(),
                actual: e.to_string(),
            })?;
        ...
        if let Some(version) = config_value.get_mut("version") {
            *version = serde_json::Value::String(migration.to_version.clone());
            changes.push(ConfigChange {
                ...
                old_value: Some(serde_json::Value::String(migration.from_version.clone())),
                new_value: Some(serde_json::Value::String(migration.to_version.clone())),
            });
        }
                ...
                if let Some(section_name) = step.parameters.get("section_name") {
                    if let Some(section_value) = step.parameters.get("section_value") {
                        ...
                        changes.push(ConfigChange {
                            ...
                            old_value: None,
                            new_value: Some(section_value.clone()),
                        });
                    }
                }
            }
