...
use super::{Config, ConfigEnvironment, ConfigAuditLog, ConfigHealth, ConfigStats, CURRENT_CONFIG_VERSION};
...
impl Config for GlobalConfig {
    ...
    fn validate_config(&self) -> crate::RhemaResult<()> {
        ...
        if self.version.is_empty() {
            return Err(crate::RhemaError::ConfigError("Version cannot be empty".to_string()));
        }
        Ok(())
    }
    
    fn load_from_file(_path: &Path) -> crate::RhemaResult<Self> {
        Self::load()
    }
    
    fn save_to_file(&self, _path: &Path) -> crate::RhemaResult<()> {
        ...
        match format {
            ...
            _ => {
                Err(crate::RhemaError::ConfigError(
                    format!("Unsupported export format: {}. Supported formats: json, yaml", format)
                ))
            }
        }
    }
        ...
        let imported_config: GlobalConfig = match format {
            ...
            _ => {
                return Err(crate::RhemaError::ConfigError(
                    format!("Unsupported import format: {}. Supported formats: json, yaml", format)
                ));
            }
        };
...
impl Default for PathConfig {
    fn default() -> Self {
        Self {
            ...
            config: dirs::config_dir().unwrap_or_else(|| PathBuf::from("~/.config")).join("rhema"),
            data: dirs::data_dir().unwrap_or_else(|| PathBuf::from("~/.local/share")).join("rhema"),
            cache: dirs::cache_dir().unwrap_or_else(|| PathBuf::from("~/.cache")).join("rhema"),
            log: dirs::data_dir().unwrap_or_else(|| PathBuf::from("~/.local/share")).join("rhema/logs"),
