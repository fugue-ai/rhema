...
pub use error::{GacpError, GacpResult};
pub use schema::*;
pub use scope::*;
...
/// Main GACP context manager
pub struct Gacp {
    repo_root: PathBuf,
}

impl Gacp {
    /// Create a new GACP instance for the current repository
    pub fn new() -> Result<Self> {
        let repo_root = git::find_repo_root()?;
        Ok(Self { repo_root })
    }

    /// Create a new GACP instance for a specific repository path
    pub fn new_from_path(repo_root: PathBuf) -> Result<Self> {
        // Verify that the path contains a git repository
        if !repo_root.join(".git").exists() {
            return Err(crate::GacpError::GitRepoNotFound(
