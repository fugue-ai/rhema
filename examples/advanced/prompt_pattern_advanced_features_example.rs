...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    variables.insert(
        ...
        "security and performance".to_string(),
    );
    ...
    let security_rule = ContextRule::new(
        ...
        vec![
            "security-patterns.yaml".to_string(),
            "vulnerability-db.yaml".to_string(),
        ],
    ...
    security_vars.insert(
        "SECURITY_CHECKS".to_string(),
        "SQL injection, XSS, buffer overflows".to_string(),
    );
    security_vars.insert(
    ...
    let high_severity_rules =
        code_review_pattern.get_matching_rules(Some("code_review"), None, Some("high"));
    println!(
        "   High severity code review matches {} rules:",
        high_severity_rules.len()
    );
    ...
    let regular_rules = code_review_pattern.get_matching_rules(Some("code_review"), None, None);
    println!(
        "   Regular code review matches {} rules:",
        regular_rules.len()
    );
    ...
    bug_vars.insert(
        ...
        "error handling, edge cases".to_string(),
    );
    ...
    let bug_rule = ContextRule::new(
        "task_type == 'bug_fix'",
        vec![
            "bug-patterns.yaml".to_string(),
            "error-handling.yaml".to_string(),
        ],
        ContextInjectionMethod::Prepend,
    );
    ...
    println!(
        "   Code review pattern supports multi-file context: {}",
...
mod tests {
    ...
    fn test_advanced_prompt_pattern_creation() {
        ...
        assert_eq!(
            pattern.get_matching_rules(Some("test"), None, None).len(),
            1
        );
