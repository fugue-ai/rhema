...
fn test_query_execution_complex_filter() -> RhemaResult<()> {
    ...
    let result = env
        .rhema
        .query("todos WHERE status=pending AND priority=high")?;
...
fn test_scope_data_files() -> RhemaResult<()> {
    ...
    assert!(data_files
        .iter()
        .any(|f| f.file_name().unwrap() == "todos.yaml"));
    assert!(data_files
        .iter()
        .any(|f| f.file_name().unwrap() == "insights.yaml"));
...
fn test_scope_schema_files() -> RhemaResult<()> {
    ...
    assert!(schema_files
        .iter()
        .any(|f| f.file_name().unwrap() == "todo.yaml"));
...
fn test_scope_discovery_performance() -> RhemaResult<()> {
    ...
    let (_, duration) =
        tests::common::performance::measure_time(|| env.rhema.discover_scopes().unwrap());
...
fn test_query_execution_performance() -> RhemaResult<()> {
    ...
    let (_, duration) =
        tests::common::performance::measure_time(|| env.rhema.query("todos").unwrap());
