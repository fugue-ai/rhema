...
export class RhemaProvider implements 
    ...
    private async checkRhemaInstallation(): Promise<void> {
        try {
            ...
            vscode.window.showWarningMessage('Rhema CLI not found. Please install Rhema CLI to use this extension.');
        }
    }
    ...
    private findSymbolInDocument(document: vscode.TextDocument, symbolName: string): vscode.Location[] {
    ...
    private async findDefinitionInWorkspace(symbolName: string): Promise<vscode.Location | undefined> {
        try {
    ...
    provideCodeActions(document: vscode.TextDocument, range: vscode.Range | vscode.Selection, context: vscode.CodeActionContext, token: vscode.CancellationToken): vscode.ProviderResult<(vscode.CodeAction | vscode.Command)[]> {
        ...
        try {
            ...
            if (context.diagnostics.length > 0) {
                const fixAction = new vscode.CodeAction('Fix Rhema Validation Issues', vscode.CodeActionKind.QuickFix);
                fixAction.command = {
            ...
            const refactorAction = new vscode.CodeAction('Refactor Rhema Context', vscode.CodeActionKind.Refactor);
            ...
            // Add source actions
            const sourceAction = new vscode.CodeAction('Generate Rhema Documentation', vscode.CodeActionKind.Source);
    ...
    provideFoldingRanges(document: vscode.TextDocument, context: vscode.FoldingContext, token: vscode.CancellationToken): vscode.ProviderResult<vscode.FoldingRange[]> {
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line === 'scope:' || line === 'context:' || line === 'todos:' || 
                    line === 'insights:' || line === 'patterns:' || line === 'decisions:') {
                    if (startLine !== -1) {
            ...
            return positions.map(position => {
                ...
                if (wordRange) {
                    ...
                    return new vscode.SelectionRange(
                        wordRange,
                        [new vscode.SelectionRange(lineRange)]
                    );
                }
        ...
        try {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (fileMatch) {
                    ...
                    link.target = vscode.Uri.file(path.resolve(path.dirname(document.fileName), fileMatch[2]));
                    links.push(link);
                }
