...
export class RhemaSettings {
  ...
  private _asyncCompletions: boolean = true;
  private _maxConcurrentCompletions: number = 3;
  private _completionTimeout: number = 10000;
  ...
  private async loadSettings(): Promise<void> {
    ...
    this._asyncCompletions = config.get('asyncCompletions', true);
    this._maxConcurrentCompletions = config.get('maxConcurrentCompletions', 3);
    this._completionTimeout = config.get('completionTimeout', 10000);
  ...
  isAsyncCompletionsEnabled(): boolean {
    return this._asyncCompletions;
  }
  getMaxConcurrentCompletions(): number {
    return this._maxConcurrentCompletions;
  }
  getCompletionTimeout(): number {
    return this._completionTimeout;
  }
