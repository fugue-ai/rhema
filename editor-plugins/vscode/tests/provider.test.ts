...
`;
    ...
    test('Definition Provider - Find Definition in Document', async () => {
        ...
        const definition = await provider.provideDefinition(testDocument, position, {} as vscode.CancellationToken);
    ...
    test('Reference Provider - Find References in Document', async () => {
        ...
        const references = await provider.provideReferences(testDocument, position, context, {} as vscode.CancellationToken);
        
        assert.ok(references.length > 0, 'Should find references');
        assert.ok(references.some(ref => ref.uri === testDocument.uri), 'Should find reference in current document');
    });

    test('Document Symbol Provider - Extract Symbols', async () => {
        const symbols = await provider.provideDocumentSymbols(testDocument, {} as vscode.CancellationToken);
    ...
    test('Workspace Symbol Provider - Search Symbols', async () => {
        ...
        assert.ok(symbols.some(s => s.name.toLowerCase().includes('scope')), 'Should find scope-related symbols');
    });
    ...
    test('Code Actions Provider - Provide Actions', async () => {
        ...
        const actions = await provider.provideCodeActions(testDocument, range, context, {} as vscode.CancellationToken);
        
        assert.ok(actions.length > 0, 'Should provide code actions');
        
        // Check for different action types
        const actionTitles = actions.map(a => a.title);
        assert.ok(actionTitles.some(title => title.includes('Refactor')), 'Should include refactoring action');
        assert.ok(actionTitles.some(title => title.includes('Documentation')), 'Should include documentation action');
    });
    ...
    test('Code Actions Provider - Quick Fix for Diagnostics', async () => {
        ...
        assert.ok(actions.some(a => a.title.includes('Fix')), 'Should include fix action');
    });

    test('Folding Range Provider - Provide Folding Ranges', async () => {
        const context = {} as vscode.FoldingContext;
        const ranges = await provider.provideFoldingRanges(testDocument, context, {} as vscode.CancellationToken);
    ...
    test('Selection Range Provider - Provide Selection Ranges', async () => {
        ...
        const ranges = await provider.provideSelectionRanges(testDocument, positions, {} as vscode.CancellationToken);
    ...
    test('Document Highlight Provider - Highlight Symbols', async () => {
        const position = new vscode.Position(2, 8); // Position at "name"
        const highlights = await provider.provideDocumentHighlights(testDocument, position, {} as vscode.CancellationToken);
    ...
    test('Document Link Provider - Find Links', async () => {
        ...
        // Check for URL links
        const urlLinks = links.filter(link => link.target?.scheme === 'http' || link.target?.scheme === 'https');
    ...
    test('Rename Provider - Provide Rename Edits', async () => {
        ...
        const edit = await provider.provideRenameEdits(testDocument, position, newName, {} as vscode.CancellationToken);
    ...
    test('Format on Type Provider - Auto Formatting', async () => {
        ...
        const edits = await provider.provideOnTypeFormattingEdits(testDocument, position, ch, options, {} as vscode.CancellationToken);
    ...
    test('Non-Rhema File Handling', async () => {
        ...
        const definition = await provider.provideDefinition(nonRhemaDocument, position, {} as vscode.CancellationToken);
        const references = await provider.provideReferences(nonRhemaDocument, position, {} as vscode.ReferenceContext, {} as vscode.CancellationToken);
        const symbols = await provider.provideDocumentSymbols(nonRhemaDocument, {} as vscode.CancellationToken);
        const actions = await provider.provideCodeActions(nonRhemaDocument, new vscode.Range(0, 0, 1, 0), {} as vscode.CodeActionContext, {} as vscode.CancellationToken);
        const ranges = await provider.provideFoldingRanges(nonRhemaDocument, {} as vscode.FoldingContext, {} as vscode.CancellationToken);
        const highlights = await provider.provideDocumentHighlights(nonRhemaDocument, position, {} as vscode.CancellationToken);
        const links = await provider.provideDocumentLinks(nonRhemaDocument, {} as vscode.CancellationToken);
        const rename = await provider.provideRenameEdits(nonRhemaDocument, position, 'newName', {} as vscode.CancellationToken);
        const format = await provider.provideOnTypeFormattingEdits(nonRhemaDocument, position, ':', {} as vscode.FormattingOptions, {} as vscode.CancellationToken);
